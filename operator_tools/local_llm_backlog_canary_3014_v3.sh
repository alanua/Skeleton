...
ISSUE="3014"
MARKER="<!-- local-llm-backlog-canary-v3 -->"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
...
if [ ! -s "$TMP/models.json" ]; then
  post_report "$MARKER
### Local LLM backlog canary v3
- status: BLOCKED
- reason: ollama_api_unreachable
- side_effects: zero"
  echo "RESULT=LOCAL_LLM_CANARY_V3_PUBLISHED"
  exit 0
fi
...
: >"$TMP/issues.txt"
for n in "${ISSUES[@]}"; do
  gh issue view "$n" -R "$REPO" --json number,title,state,labels,body >"$TMP/issue-$n.json"
  python3 - "$TMP/issue-$n.json" <<'PY' >>"$TMP/issues.txt"
import json,sys
with open(sys.argv[1], encoding='utf-8') as f:
    x=json.load(f)
labels=','.join(i.get('name','') for i in x.get('labels',[]))
body=' '.join((x.get('body') or '').split())[:500]
print('#{} | state={} | labels={} | title={} | summary={}'.format(
    x.get('number'), x.get('state'), labels, x.get('title',''), body
))
PY
done
...
prompt='''Classify each GitHub issue using ONLY its supplied public metadata. Allowed classes: ACTIVE, WAIT_DEPENDENCY, SUPERSEDED, NEEDS_REVIEW, SAFE_NO_MODEL. Output one line per issue exactly as: #1234 | CLASS. Do not explain. Do not omit an issue.\n\n'''+text
...
for n,c in re.findall(r'#?(\d{4})\s*\|\s*(ACTIVE|WAIT_DEPENDENCY|SUPERSEDED|NEEDS_REVIEW|SAFE_NO_MODEL)',raw.upper()):
    n=int(n)
    if n in expected and c in allowed:
        found[n]=c
valid=set(found)==expected
print('<!-- local-llm-backlog-canary-v3 -->')
print('### Local LLM backlog canary v3')
print('- status: {}'.format('PASS' if valid else 'BLOCKED'))
print('- local_runtime: available')
print('- local_model: `{}`'.format(model))
print('- elapsed_seconds: `{}`'.format(elapsed))
print('- parsed_items: `{}/5`'.format(len(found)))
...
if found:
    ...
    for n in sorted(found):
        print('- #{}: `{}`'.format(n,found[n]))
if not valid:
    print('- reason: incomplete_simple_contract')
