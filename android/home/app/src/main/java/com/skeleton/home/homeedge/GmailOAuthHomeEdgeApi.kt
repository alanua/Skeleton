...
class GmailOAuthHomeEdgeApi(
    configuredBaseUrls: String = BuildConfig.HOME_EDGE_BASE_URLS,
    private val transport: (baseUrl: String, path: String, payload: String?) -> String = ::httpRequest,
) {
    ...
    private fun <T> requestFirst(path: String, payload: String?, decode: (String) -> T): T {
        ...
        for (baseUrl in baseUrls) {
            try {
                return decode(transport(baseUrl, path, payload))
    ...
    companion object {
        private fun httpRequest(baseUrl: String, path: String, payload: String?): String {
