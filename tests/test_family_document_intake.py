...
def make_processor(tmp_path: Path, text_by_name: dict[str, str], *, calendar_fail: bool = False):
    ...
    def memory_runner(command, input_text, timeout, max_output):
        ...
        return 0, json.dumps({"payload": {"status": "DONE", "authoritative": True}}), ""
    ...
    processor = DocumentProcessor(
        IntakeConfig(
            ...
            memory_sink=MemoryGatewaySink(JsonCommandAdapter(("/usr/bin/true",), runner=memory_runner), approval_ref="operator.family.test"),
...
def test_complete_private_record_and_semantic_event_date(tmp_path: Path) -> None:
    ...
    assert set(record["field_confidence"]) >= {"principal_subject", "topic", "jurisdiction", "document_type", "issuer", "identifiers", "amounts", "deadlines"}
...
def test_ambiguous_subject_routes_to_review_without_side_effects(tmp_path: Path) -> None:
    ...
    assert receipt["reason_code"] == "review_required"
...
def test_reconciliation_packet_is_deterministic_zero_side_effect_and_clusters(tmp_path: Path) -> None:
    text = ready_text()
    processor, inbox, archive, calls, calendar_calls = make_processor(tmp_path, {"a.pdf": text, "b.pdf": text, "c.pdf": text.replace("25.07.2026", "26.07.2026")})
