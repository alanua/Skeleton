...
from core.family_document_runtime import DurableJournal, FamilyDocumentWorker, ProjectionOutbox, ReceiptOutbox, RuntimeLimits
...
def make_processor(tmp_path: Path, text_by_name: dict[str, str], *, calendar_fail: bool = False):
    ...
    processor = DocumentProcessor(
        IntakeConfig(
            ...
            receipt_outbox=ReceiptOutbox(tmp_path / "receipt-outbox.json"),
...
def test_notification_transport_failure_does_not_rollback_document_commit(tmp_path: Path) -> None:
    processor, inbox, archive, calls, _ = make_processor(tmp_path, {"scan.pdf": ready_text()})
    source = inbox / "scan.pdf"
    source.write_bytes(b"binary")
    journal = DurableJournal(
        tmp_path / "journal.json",
        RuntimeLimits(settle_seconds=1, lease_seconds=30, retry_base_seconds=1),
    )
    ...
    __import__("time").sleep(1.01)
    assert worker.run_once()["operation"] == "PROCESSED"
    committed_calls = list(calls)
    archive_files = [path for path in archive.rglob("*") if path.is_file()]
    assert len(archive_files) == 1
    assert committed_calls[0]["command"] == "skeleton.memory.private_mutate"
    assert committed_calls[1]["command"] == "skeleton.memory.private_read_exact"

    result = processor.config.receipt_outbox.process_one(
        lambda packet: (_ for _ in ()).throw(RuntimeError("telegram unavailable")),
        max_attempts=2,
    )
    assert result == "RETRY"
    assert calls == committed_calls
    assert [path for path in archive.rglob("*") if path.is_file()] == archive_files
    assert processor.config.receipt_outbox.counts() == {"PENDING": 1, "RETRY": 1}
