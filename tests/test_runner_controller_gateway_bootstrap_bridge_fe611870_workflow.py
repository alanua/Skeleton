...
EXPECTED_TRIGGER_LABEL = "gateway-bootstrap-fe611870-approved"
...
ALLOWED_CHANGED_FILES = {
    ...
    "tests/test_runner_controller_gateway_bootstrap_bridge_fe611870_workflow.py",
}
...
def test_operator_gate_binds_exact_base_branch_and_current_head() -> None:
    ...
    assert f"github.event.label.name == '{EXPECTED_TRIGGER_LABEL}'" in condition
...
def test_legacy_operator_gate_values_are_not_reused() -> None:
    text = _text()
    assert "github.event.label.name == 'gateway-bootstrap-approved'" not in text
    assert (
        "Authorization Marker: "
        "EXACT_HEAD_RUNNER_CONTROLLER_GATEWAY_BOOTSTRAP_BRIDGE_APPROVED"
    ) not in text
    assert "fe611870" in EXPECTED_TRIGGER_LABEL
    assert "FE611870" in EXPECTED_MARKER


def test_root_mutation_is_only_exact_bootstrap_install_and_execution() -> None:
    text = _text()
    assert text.count("/usr/bin/sudo -n") == 2
    assert "/usr/bin/sudo -n /usr/bin/install -D -o root -g root -m 0555 --" in text
    assert '/usr/bin/sudo -n "${PROTECTED_BOOTSTRAP_PATH}"' in text
    assert '--repo-root "${TARGET_CHECKOUT}"' in text
    assert '--expected-main-sha "${EXPECTED_MAIN_SHA}"' in text
    assert (
        "PROTECTED_BOOTSTRAP_PATH: "
...
def test_source_is_exact_git_object_and_main_is_reverified_before_root() -> None:
    ...
    assert f"EXPECTED_BOOTSTRAP_BLOB: {EXPECTED_BOOTSTRAP_BLOB}" in text
    assert 'test "${bootstrap_tree_blob}" = "${EXPECTED_BOOTSTRAP_BLOB}"' in text
...
def test_bridge_cannot_mutate_ssh_authentication() -> None:
    text = _text().lower()
    forbidden = (
        "--ssh-public-key",
        "authorized_keys",
...
def test_expected_scope_is_exact_two_files() -> None:
    for relative_path in ALLOWED_CHANGED_FILES:
        assert (ROOT / relative_path).exists()
    assert WORKFLOW.name.endswith("-fe611870.yml")
