...
EXPECTED_BRANCH = "runner/issue-3517"
...
def _job() -> dict[str, object]:
    job = _workflow()["jobs"]["host-self-audit"]
    assert isinstance(job, dict)
    return job


def _anchor_matrix() -> dict[str, tuple[str, str, str, str]]:
    matrix = _job()["env"]["ANCHOR_MATRIX"]
...
def test_canonical_gateway_trust_anchor_matrix_matches_exact_main_blobs() -> None:
    assert _job()["env"]["EXPECTED_MAIN_SHA"] == EXPECTED_MAIN_SHA
...
def test_summary_receipt_contains_exact_authorization_without_private_outputs() -> None:
    text = _text()
    ...
    assert 'echo "authorization_label=${EXPECTED_LABEL}"' in summary
    assert 'echo "authorization_marker=${EXPECTED_MARKER}"' in summary
    assert 'echo "authorization_head_ref=${EXPECTED_BRANCH}"' in summary
    assert 'echo "authorized_head_sha=${{ github.event.pull_request.head.sha }}"' in summary
...
def test_summary_receipt_contains_semantic_pass_matrix() -> None:
    text = _text()
    summary_start = text.index('echo "status=${status}"')
    summary_end = text.index('} >> "${GITHUB_STEP_SUMMARY}"', summary_start)
    summary = text[summary_start:summary_end]
