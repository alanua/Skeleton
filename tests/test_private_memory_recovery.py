...
def _drop_protection_triggers(connection: sqlite3.Connection) -> None:
    for trigger_name in (
...
def test_weakened_same_column_fact_table_definition_blocks_integrity(
    tmp_path: Path,
) -> None:
    store = _store(tmp_path)
    _write_fact(store, {"label": "alpha"})
    db_path = tmp_path / "synthetic-memory.sqlite"
    with sqlite3.connect(db_path) as connection:
        _drop_protection_triggers(connection)
        connection.execute("ALTER TABLE private_memory_facts RENAME TO private_memory_facts_old")
        connection.execute(
            """
        ...
        connection.execute(
            ...
            INSERT INTO private_memory_facts (
                namespace, fact_id, value_json, value_hash, created_at, updated_at,
                canonical_revision, tombstoned_at, tombstone_reason
            )
            SELECT namespace, fact_id, value_json, value_hash, created_at, updated_at,
                canonical_revision, tombstoned_at, tombstone_reason
            FROM private_memory_facts_old
            """
        )
        connection.execute("DROP TABLE private_memory_facts_old")
...
def test_weakened_same_column_event_table_definition_blocks_integrity(
    tmp_path: Path,
) -> None:
    store = _store(tmp_path)
    _write_fact(store, {"label": "alpha"})
    ...
    with sqlite3.connect(db_path) as connection:
        _drop_protection_triggers(connection)
        connection.execute("ALTER TABLE private_memory_events RENAME TO private_memory_events_old")
        connection.execute(
            """
        ...
        connection.execute(
            ...
            INSERT INTO private_memory_events (
                event_id, schema, event_type, namespace, fact_id, actor_ref,
                reason_code, approval_ref, transaction_ref, timestamp,
                previous_hash, new_hash, canonical_revision
            )
            SELECT event_id, schema, event_type, namespace, fact_id, actor_ref,
                reason_code, approval_ref, transaction_ref, timestamp,
                previous_hash, new_hash, canonical_revision
            FROM private_memory_events_old
            """
        )
        connection.execute("DROP TABLE private_memory_events_old")
        ensure_history_schema(connection)
...
def test_duplicate_current_fact_rows_fail_integrity(tmp_path: Path) -> None:
    store = _store(tmp_path)
    _write_fact(store, {"label": "alpha"})
    db_path = tmp_path / "synthetic-memory.sqlite"
    with sqlite3.connect(db_path) as connection:
...
@pytest.mark.parametrize(
    ("trigger_name", "tamper_sql", "params"),
    [
        (
            None,
...
def test_public_read_routes_fail_closed_after_canonical_tampering(
    tmp_path: Path,
    ...
    with sqlite3.connect(tmp_path / "synthetic-memory.sqlite") as connection:
        ...
        connection.execute(tamper_sql, params)

    for read in (
        lambda: store.current_revision(),
        lambda: store.get_active_fact(namespace="synthetic", fact_id="fact-001"),
        lambda: store.history(namespace="synthetic", fact_id="fact-001"),
    ):
        with pytest.raises(Exception):
            read()
