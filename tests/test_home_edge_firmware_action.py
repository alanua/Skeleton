...
def test_missing_secret_files_block_before_transfer(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_strict_scp_and_ssh_are_fixed(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        ...
        return 0, json.dumps(
            {
                ...
                "sha256": _request(tmp_path).sha256,
                "byte_size": _request(tmp_path).byte_size,
                "preflight_state": "ok",
    ...
    assert scp_args[:6] == ["scp", "-q", "-o", "BatchMode=yes", "-o", "StrictHostKeyChecking=yes"]
    ...
    assert "arbitrary" not in str(ssh_stdin)
...
def test_remote_unverified_blocks_without_private_values(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    identity = tmp_path / "id_ed25519"
    known_hosts = tmp_path / "known_hosts"
    identity.write_text("identity", encoding="utf-8")
    known_hosts.write_text("host key", encoding="utf-8")
