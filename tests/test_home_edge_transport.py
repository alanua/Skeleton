...
def test_openssh_argv_is_exact_and_profile_derived(tmp_path: Path) -> None:
...
def test_connection_fields_ignore_untrusted_values(tmp_path: Path) -> None:
    ...
    assert argv[-3:] == ["runtime-user@100.64.0.88", "python3", "-"]
...
def test_tailscale_adapter_is_explicit_and_interactive() -> None:
    profile = load_home_edge_profile(environment={"SKELETON_HOME_EDGE_01_TARGET_USER": "runtime-user", "SKELETON_HOME_EDGE_01_HOSTNAME": "runtime-host"})
    transport = TailscaleSSHTransport(profile=profile)

    assert transport.adapter_name == "tailscale_ssh_interactive"
    assert transport.build_probe_argv() == [
        "tailscale",
        "ssh",
        "runtime-user@runtime-host",
        "python3",
        "-",
    ]
