...
def test_recovery_store_survives_modeled_repo_codex_cleanup_and_preserves_attempt(
    tmp_path: Path,
) -> None:
...
def test_recovery_store_rejects_symlink_state_directory(tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError, match="CONTROL_RECOVERY_STATE_DIR_INVALID"):
...
def test_recovery_store_rejects_symlink_state_file(tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError, match="CONTROL_RECOVERY_STATE_FILE_INVALID"):
