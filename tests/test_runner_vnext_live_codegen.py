...
def runtime(context, *, manifest_adapter_id: str | None = None):
    adapter_id = manifest_adapter_id or context.adapter_id
    ...
    nodes.register(NodeCapabilitySnapshot(
        ...
        supported_adapters=(adapter_id,), supported_lanes=(Lane.CODEGEN,),
...
def test_compiler_binds_source_adapter_and_runner_task_hash_into_vnext_identity_and_target_state() -> None:
    context = compile_repository_codegen_context(task_mapping(), source_task_ref="issue:4085")
    runner_task_hash = context.runner_task_json_sha256
    assert runner_task_hash == hashlib.sha256(context.runner_task.to_json().encode("utf-8")).hexdigest()
    assert context.task_id.endswith(context.binding_hash)
    assert context.operation_id.endswith(context.binding_hash)
    assert context.idempotency_key.endswith(context.binding_hash)
    assert context.binding_hash != runner_task_hash
    assert f"runner-task-sha256:{runner_task_hash}" in context.universal_task.operator_boundary_evidence
    assert context.target_state_ref == f"git:{BASE_SHA}"
    assert context.resources == ("repo:core/example.py", "repo:tests/test_example.py")

    same_task_other_source = compile_repository_codegen_context(task_mapping(), source_task_ref="issue:4086")
    same_task_other_adapter = compile_repository_codegen_context(task_mapping(), source_task_ref="issue:4085", adapter_id="adapter:repository-codegen-alt")
    assert same_task_other_source.runner_task_json_sha256 == runner_task_hash
    assert same_task_other_adapter.runner_task_json_sha256 == runner_task_hash
    assert len({context.task_id, same_task_other_source.task_id}) == 2
    assert len({context.operation_id, same_task_other_source.operation_id, same_task_other_adapter.operation_id}) == 3
    assert len({context.idempotency_key, same_task_other_source.idempotency_key, same_task_other_adapter.idempotency_key}) == 3


def test_runner_task_field_changes_change_raw_hash_and_composite_binding() -> None:
    variants = (
        task_mapping(payload={"operation": "other"}),
        task_mapping(allowed_files=("core/example.py",)),
        task_mapping(validation_commands=(("python3", "-m", "py_compile", "core/example.py"),)),
        task_mapping(idempotency_key="repo-codegen-other"),
        task_mapping(base_sha="f" * 40),
        task_mapping(branch="runner/other"),
        task_mapping(requested_capabilities=("repository_read", "repository_write_allowlisted", "test_execution", "subprocess_isolated")),
    )
    contexts = [compile_repository_codegen_context(value, source_task_ref="issue:4085") for value in (task_mapping(), *variants)]
    assert len({context.runner_task_json_sha256 for context in contexts}) == len(contexts)
    assert len({context.binding_hash for context in contexts}) == len(contexts)
...
def test_live_codegen_rejects_non_public_code_edit_or_canonical_protected_targets_before_backend() -> None:
    with pytest.raises(RepositoryCodegenCompileError, match="REPOSITORY_CODEGEN_TASK_KIND_UNSUPPORTED"):
        compile_repository_codegen_context(task_mapping(task_kind="diagnostic"), source_task_ref="issue:4085")
    with pytest.raises(RepositoryCodegenCompileError, match="REPOSITORY_CODEGEN_PRIVACY_BOUNDARY_UNSUPPORTED"):
        compile_repository_codegen_context(task_mapping(privacy_boundary="PUBLIC_SAFE_AGGREGATE_ONLY"), source_task_ref="issue:4085")
    for protected in ("scripts/runner_poll_github_tasks.py", "core/runner_vnext_execution.py"):
        with pytest.raises(RepositoryCodegenCompileError, match="REPOSITORY_CODEGEN_PROTECTED_RESOURCE"):
            compile_repository_codegen_context(task_mapping(allowed_files=(protected,)), source_task_ref="issue:4085")


def test_grant_bound_adapter_calls_backend_with_exact_task_capabilities_and_truthful_touched_subset() -> None:
    context = compile_repository_codegen_context(task_mapping(), source_task_ref="issue:4085")
    ...
    assert grant.planner_envelope.required_capabilities == context.runner_task.requested_capabilities
...
def test_swapped_context_or_mutated_grant_capabilities_fail_before_backend_call() -> None:
    context = compile_repository_codegen_context(task_mapping(), source_task_ref="issue:4085")
    scheduler, ledger, grant = runtime(context)
    swapped = compile_repository_codegen_context(task_mapping(payload={"operation": "other"}), source_task_ref="issue:4085")
    backend = Backend()
    ...
    context = compile_repository_codegen_context(task_mapping(), source_task_ref="issue:4085")
    scheduler, ledger, grant = runtime(context)
    forged_envelope = replace(grant.planner_envelope, required_capabilities=("repository_write_allowlisted",))
    forged_grant = replace(grant, planner_envelope=forged_envelope)
    backend = Backend()
    ...
    with pytest.raises(GreenExecutionError, match="REPOSITORY_CODEGEN_GRANT_BINDING_MISMATCH"):
        RepositoryCodegenExecutorAdapter(context=context, backend=backend).execute(forged_grant)
    assert backend.calls == 0


def test_backend_touched_resources_are_fail_closed_for_outside_duplicate_malformed_or_zero_mutation() -> None:
    context = compile_repository_codegen_context(task_mapping(), source_task_ref="issue:4085")
    bad_results = (
        ...
        RepositoryCodegenBackendResult(context.operation_id, context.idempotency_key, context.adapter_id, context.target_state_ref, 1, ("repo:../escape.py",), "SUCCEEDED", "EXECUTION_PASS", context.target_state_ref, "git:b", "PASS"),
...
def test_compiler_accepts_already_validated_runner_task_without_reordering_drift() -> None:
    ...
    context = compile_repository_codegen_context(task, source_task_ref="issue:4085")
