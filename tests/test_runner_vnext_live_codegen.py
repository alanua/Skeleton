...
def test_changed_payload_allowed_files_validation_or_idempotency_changes_binding() -> None:
    ...
    variants = [
        ...
        runner_task(allowed_files=["core/example.py"]),
...
def test_canonical_exact_protected_allowed_file_fails_before_backend_invocation(path: str) -> None:
    task = runner_task(allowed_files=[path])
...
def test_canonical_protected_prefix_allowed_file_fails_before_backend_invocation(prefix: str) -> None:
...
def test_representative_ordinary_paths_remain_eligible_repo_resources(path: str) -> None:
    plan = compile_repository_codegen_plan(runner_task=runner_task(allowed_files=[path]), source_task_ref="github-issue:4067")
    assert plan.context.authorized_resource_refs == (f"repo:{path}",)
...
def test_grant_bound_adapter_invokes_backend_with_exact_validated_task_after_grant() -> None:
    ...
    assert receipt.touched_resource_refs == plan.context.authorized_resource_refs
...
def test_swapped_runner_task_against_existing_grant_fails_before_backend() -> None:
    ...
    assert isinstance(exc.value.__cause__, GreenExecutionError)
    assert exc.value.__cause__.reason_code == "REPOSITORY_CODEGEN_TASK_HASH_MISMATCH"
...
def test_backend_output_outside_scope_fails_after_effect_and_needs_recovery() -> None:
    ...
    assert isinstance(exc.value.__cause__, GreenExecutionError)
    assert exc.value.__cause__.reason_code == "REPOSITORY_CODEGEN_TOUCHED_RESOURCE_OUT_OF_SCOPE"
    assert backend.calls == 1
    assert ledger.status(grant.idempotency_key) == "STARTED"
