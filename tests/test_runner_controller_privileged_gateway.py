...
SSH_KEY = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA= runner@example"
...
def _run_installer(repo: Path, destdir: Path, sha: str, *extra: str, env: dict[str, str] | None = None) -> subprocess.CompletedProcess[str]:
    child_env = {
        ...
        "SKELETON_GATEWAY_SYNTHETIC_REMOTE_MAIN": sha,
    }
    if env:
...
def test_ssh_restrictions_and_root_login_prohibition_are_deterministic() -> None:
    ...
    assert line.startswith(
        ...
        "restrict,no-pty,no-port-forwarding,no-X11-forwarding,no-agent-forwarding,no-user-rc "
    )
    ...
    assert "DisableForwarding yes" in fragment
    assert "AllowTcpForwarding no" in fragment
    assert "AllowStreamLocalForwarding no" in fragment
...
def test_wrong_sha_wrong_origin_stale_origin_fresh_remote_and_installer_mismatch_block_before_install(tmp_path: Path) -> None:
    ...
    assert wrong_origin.returncode == 0, wrong_origin.stderr
...
def test_key_path_installs_exact_account_key_sudoers_match_and_no_root_policy(tmp_path: Path) -> None:
    ...
    assert (dest / "etc/passwd.d/skeleton-runner-gateway.plan").read_text(encoding="utf-8").endswith(":/nonexistent:/bin/sh\n")
    assert (dest / "var/lib/skeleton/runner-controller/ssh/authorized_keys").read_text(encoding="utf-8") == (
        f'command="/usr/bin/sudo -n /usr/local/libexec/skeleton/runner-controller/privileged-gateway",restrict,no-pty,no-port-forwarding,no-X11-forwarding,no-agent-forwarding,no-user-rc {SSH_KEY}\n'
    )
