...
def test_resolves_existing_canonical_config_without_duplicate_env(tmp_path: Path) -> None:
    root = _canonical_root(tmp_path)
    resolved, source = resolve_private_memory_root(
        {"SKELETON_PRIVATE_MEMORY_CONFIG": str(_config(tmp_path, root / "canonical.sqlite"))},
...
def test_legacy_connector_database_only_anchors_private_directory(tmp_path: Path) -> None:
    ...
    resolved, source = resolve_private_memory_root(
        {"SKELETON_PRIVATE_MEMORY_CONFIG": str(_config(tmp_path, legacy_database))},
        checkout=tmp_path / "checkout",
    )
    assert resolved == str(root.resolve())
    assert source == "private_config_parent"
...
def test_canonical_resolver_accepts_only_canonical_config_database(tmp_path: Path) -> None:
...
def test_canonical_resolver_never_creates_missing_default(tmp_path: Path) -> None:
    ...
    with pytest.raises(PrivateMemoryRootResolutionError) as exc_info:
        resolve_canonical_stack_root({}, checkout=tmp_path / "checkout", home=home)
