...
CLEANUP_PR_BODY = "Automated Runner task from issue #2145."
...
def test_runner_labels_repository_issue_gate_and_source_base_constant_are_fixed() -> None:
    ...
    pull_request_authorization = (
        ...
        f"github.event.pull_request.body == '{CLEANUP_PR_BODY}'"
    )
    assert job["if"] == (
...
def test_cleanup_pr_body_gate_is_exact_deterministic_equality() -> None:
    job_if = workflow_text()
    old_cleanup_marker = "Authorization Marker: " + "_".join(
        ["EXPLICIT", "RUNTIME", "RECOVERY", "CLEANUP", "TRIGGER", "20260803"]
    )

    assert f"github.event.pull_request.body == '{CLEANUP_PR_BODY}'" in job_if
    assert "contains(github.event.pull_request.body" not in job_if
    assert old_cleanup_marker not in job_if
    for rejected_body in [
        "",
        "Automated Runner task from issue #2144.",
        f"x{CLEANUP_PR_BODY}",
        f"{CLEANUP_PR_BODY}x",
        f"{CLEANUP_PR_BODY}\nAuthorization Marker: ignored",
        f" {CLEANUP_PR_BODY}",
        f"{CLEANUP_PR_BODY} ",
    ]:
        assert f"github.event.pull_request.body == '{rejected_body}'" not in job_if


def test_job_condition_rejects_wrong_repository_action_base_branch_author_or_body() -> None:
    job_if = workflow_text()
    ...
    assert CLEANUP_PR_BODY in job_if
