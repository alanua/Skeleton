from __future__ import annotations

from core.scheduler_models import ScheduleSpec
from core.scheduler_store import SchedulerStore
from core.shared_dispatch import PRIVACY_PUBLIC_SAFE
from core.review_gate import (
    ReviewControlAdapters,
    ensure_draft_pr_review_continuation,
    schedule_verified_repair_done,
)
from core.notification_policy import operator_notification_ledger_key
from scripts.scheduler_runtime import _repair_issue_body, _repair_runner_task, run_scheduler_tick


HEAD_SHA = "a" * 40


def _review_state(*, head_sha: str = HEAD_SHA, files=None, checks=None, findings=None):
    return {
        "pr": {
            "state": "open",
            "draft": True,
            "mergeable": True,
            "mergeable_state": "clean",
            "base": {"ref": "main", "sha": "b" * 40},
            "head": {"ref": "runner/issue-2302", "sha": head_sha},
        },
        "files": [{"filename": path} for path in (files or ["docs/AUTONOMOUS_REVIEW_GATE.md"])],
        "compare": {"status": "ahead", "ahead_by": 1, "behind_by": 0},
        "combined_status": {"state": "success", "statuses": [{"state": "success"}]},
        "check_runs": checks or [{"status": "completed", "conclusion": "success"}],
        "review_findings": findings or [],
    }


def test_scheduler_runtime_runs_synthetic_loop_canary(tmp_path) -> None:
    scheduler_db = tmp_path / "scheduler.sqlite3"
    loop_db = tmp_path / "loop.sqlite3"
    store = SchedulerStore(scheduler_db)
    store.initialize()
    store.register(
        ScheduleSpec.from_mapping(
            {
                "schema": "skeleton.schedule.v1",
                "schedule_id": "runtime.canary",
                "trigger_kind": "once",
                "cron_expression": None,
                "once_at": 100,
                "timezone": "UTC",
                "route_type": "loop",
                "route_id": "loop_engine_packet",
                "approval_policy": "auto_run_low_risk",
                "overlap_policy": "queue_one",
                "misfire_policy": "run_once",
                "payload": {
                    "privacy_boundary": PRIVACY_PUBLIC_SAFE,
                    "bounded": True,
                    "requested_capabilities": ["loop:state_write"],
                    "task_packet": {
                        "schema": "skeleton.loop_runner_packet.v1",
                        "action": "create",
                        "task_id": "runtime-task",
                        "run_id": "runtime-run",
                        "recorded_at": 100,
                        "public_safe": True,
                        "no_secrets": True,
                        "no_runtime_mutation": True,
                        "authority_boundary": {
                            "operational_state_write": True,
                            "external_side_effects_allowed": False,
                            "runtime_mutation_allowed": False,
                        },
                    },
                },
            }
        ),
        now=50,
    )

    receipt = run_scheduler_tick(
        scheduler_db_path=str(scheduler_db),
        loop_state_db_path=str(loop_db),
        now=100,
    )

    assert receipt["dispatch"]["done"] == 1
    assert store.list_occurrences("runtime.canary")[0].state == "done"


def test_run_scheduler_tick_consumes_internal_review_without_route_block(tmp_path) -> None:
    scheduler_db = tmp_path / "scheduler.sqlite3"
    loop_db = tmp_path / "loop.sqlite3"
    continuations = []
    deliveries = []
    ensure_draft_pr_review_continuation(
        SchedulerStore(scheduler_db),
        repository="alanua/Skeleton",
        pr_number=2302,
        head_sha=HEAD_SHA,
        source_issue=2301,
        allowed_files=["docs/AUTONOMOUS_REVIEW_GATE.md"],
        now=100,
    )

    receipt = run_scheduler_tick(
        scheduler_db_path=str(scheduler_db),
        loop_state_db_path=str(loop_db),
        now=100,
        review_adapters=ReviewControlAdapters(
            state_reader=lambda payload: _review_state(),
            authorized_continuation=lambda packet: continuations.append(packet)
            or {"status": "DONE"},
            needs_operator_delivery=lambda packet: deliveries.append(packet)
            or {"status": "DONE", "adapter": "needs_operator"},
        ),
    )

    store = SchedulerStore(scheduler_db)
    receipts = [
        dispatch
        for schedule in store.list_enabled()
        for occurrence in store.list_occurrences(schedule.spec.schedule_id)
        for dispatch in store.list_dispatch_receipts(occurrence.occurrence_id)
    ]
    assert receipt["dispatch"]["done"] >= 1
    assert len(continuations) == 1
    assert deliveries == []
    assert continuations[0]["merge_policy_authority"]["verdict"] == "AUTO_MERGE_ALLOWED"
    assert "user_approved" not in continuations[0]
    assert "approval_reference" not in continuations[0]
    assert "trusted_approval_references" not in continuations[0]
    assert "trusted_protected_approval_references" not in continuations[0]
    assert all(item["reason"] != "ROUTE_NOT_ALLOWLISTED" for item in receipts)
    assert any(
        item["result"]["route_receipt"].get("internal_review_verdict") == "APPROVE"
        for item in receipts
    )


def test_production_default_adapters_safe_approve_continues_without_operator_notice(
    tmp_path, monkeypatch
) -> None:
    from scripts import runner_poll_github_tasks as runner

    scheduler_db = tmp_path / "scheduler.sqlite3"
    loop_db = tmp_path / "loop.sqlite3"
    commands = []
    sent = []
    ensure_draft_pr_review_continuation(
        SchedulerStore(scheduler_db),
        repository="alanua/Skeleton",
        pr_number=2302,
        head_sha=HEAD_SHA,
        source_issue=2301,
        allowed_files=["docs/AUTONOMOUS_REVIEW_GATE.md"],
        now=100,
    )
    monkeypatch.setattr(runner, "_get_pr_mergeability_state", lambda pr_number: _review_state())
    monkeypatch.setattr(
        runner,
        "run_command",
        lambda command, **kwargs: commands.append(command) or (1, "unexpected"),
    )
    monkeypatch.setattr(runner, "send_telegram_notification", lambda text: sent.append(text))

    run_scheduler_tick(
        scheduler_db_path=str(scheduler_db),
        loop_state_db_path=str(loop_db),
        now=100,
    )

    assert commands == []
    assert sent == []
    store = SchedulerStore(scheduler_db)
    authorized = [
        receipt
        for schedule in store.list_enabled()
        for occurrence in store.list_occurrences(schedule.spec.schedule_id)
        for receipt in store.list_dispatch_receipts(occurrence.occurrence_id)
        if receipt["result"]["route_receipt"].get("next_step")
        == "authorized_workflow_continuation"
    ]
    assert len(authorized) == 1
    route_receipt = authorized[0]["result"]["route_receipt"]
    assert route_receipt["merge_policy_authority"]["verdict"] == "AUTO_MERGE_ALLOWED"
    assert "user_approved" not in route_receipt["authorized_continuation"]
    assert "approval_reference" not in route_receipt["authorized_continuation"]


def test_production_default_adapters_request_changes_enqueue_repair_once(
    tmp_path, monkeypatch
) -> None:
    from scripts import runner_poll_github_tasks as runner

    scheduler_db = tmp_path / "scheduler.sqlite3"
    loop_db = tmp_path / "loop.sqlite3"
    commands = []
    ensure_draft_pr_review_continuation(
        SchedulerStore(scheduler_db),
        repository="alanua/Skeleton",
        pr_number=2302,
        head_sha=HEAD_SHA,
        source_issue=2301,
        allowed_files=["docs/AUTONOMOUS_REVIEW_GATE.md"],
        now=100,
    )
    monkeypatch.setattr(
        runner,
        "_get_pr_mergeability_state",
        lambda pr_number: _review_state(findings=["missing bounded test"]),
    )

    def fake_run_command(command, **kwargs):
        commands.append(command)
        if command[:3] == ["gh", "issue", "list"]:
            return 0, "{}"
        if command[:3] == ["gh", "label", "list"]:
            return 0, ""
        if command[:3] == ["gh", "label", "create"]:
            return 0, ""
        if command[:3] == ["gh", "issue", "create"]:
            return 0, "https://github.com/alanua/Skeleton/issues/1000\n"
        return 1, "unexpected command"

    monkeypatch.setattr(runner, "run_command", fake_run_command)

    first = run_scheduler_tick(
        scheduler_db_path=str(scheduler_db),
        loop_state_db_path=str(loop_db),
        now=100,
    )
    second = run_scheduler_tick(
        scheduler_db_path=str(scheduler_db),
        loop_state_db_path=str(loop_db),
        now=100,
    )

    store = SchedulerStore(scheduler_db)
    rereviews = [
        occurrence
        for schedule in store.list_enabled()
        for occurrence in store.list_occurrences(schedule.spec.schedule_id)
        if occurrence.proposal["payload"].get("repair_parent_reason")
    ]
    assert first["dispatch"]["done"] >= 1
    assert second["dispatch"]["done"] == 0
    assert [command[:3] for command in commands].count(["gh", "issue", "create"]) == 1
    assert rereviews == []


def test_repair_task_contract_rebuilds_replacement_from_current_main() -> None:
    repair_task = {
        "task_id": "repair-pr-2308-abcdef",
        "idempotency_key": "repair-key",
        "repository": "alanua/Skeleton",
        "pr_number": 2308,
        "head_sha": HEAD_SHA,
        "source_issue": 2309,
        "allowed_files": ("core/review_gate.py",),
        "reason": "review_findings_present",
        "supersedes_pr_number": 2308,
        "supersedes_head_sha": HEAD_SHA,
    }

    task = _repair_runner_task(repair_task)
    body = _repair_issue_body(repair_task, task)

    assert task.payload["operation"] == "internal_review_repair_replacement_pr"
    assert task.payload["base_policy"] == "start_from_current_main"
    assert task.payload["reviewed_pr_number"] == 2308
    assert task.payload["reviewed_head_sha"] == HEAD_SHA
    assert task.payload["replacement_pr_required"] is True
    assert "do not rely on a custom RunnerTask branch field" in task.forbidden_actions
    assert "existing PR branch" not in body
    assert "Start from current main" in body
    assert "reread the exact reviewed PR/head/diff" in body


def test_production_default_adapters_protected_pr_notifies_once(
    tmp_path, monkeypatch
) -> None:
    from scripts import runner_poll_github_tasks as runner

    scheduler_db = tmp_path / "scheduler.sqlite3"
    loop_db = tmp_path / "loop.sqlite3"
    sent = []
    ensure_draft_pr_review_continuation(
        SchedulerStore(scheduler_db),
        repository="alanua/Skeleton",
        pr_number=2302,
        head_sha=HEAD_SHA,
        source_issue=2301,
        allowed_files=["scripts/runner_poll_github_tasks.py"],
        now=100,
    )
    monkeypatch.setattr(
        runner,
        "_get_pr_mergeability_state",
        lambda pr_number: _review_state(files=["scripts/runner_poll_github_tasks.py"]),
    )
    monkeypatch.setattr(runner, "run_command", lambda command, **kwargs: (_ for _ in ()).throw(AssertionError(command)))
    monkeypatch.setattr(runner, "send_telegram_notification", lambda text: sent.append(text))

    run_scheduler_tick(
        scheduler_db_path=str(scheduler_db),
        loop_state_db_path=str(loop_db),
        now=100,
    )
    run_scheduler_tick(
        scheduler_db_path=str(scheduler_db),
        loop_state_db_path=str(loop_db),
        now=100,
    )
    run_scheduler_tick(
        scheduler_db_path=str(scheduler_db),
        loop_state_db_path=str(loop_db),
        now=100,
    )

    assert len(sent) == 1
    assert "Pull Request: #2302" in sent[0]
    assert HEAD_SHA in sent[0]


def test_production_path_request_changes_enqueues_one_repair_and_waits_for_done(tmp_path) -> None:
    scheduler_db = tmp_path / "scheduler.sqlite3"
    loop_db = tmp_path / "loop.sqlite3"
    repairs = []
    ensure_draft_pr_review_continuation(
        SchedulerStore(scheduler_db),
        repository="alanua/Skeleton",
        pr_number=2302,
        head_sha=HEAD_SHA,
        source_issue=2301,
        allowed_files=["docs/AUTONOMOUS_REVIEW_GATE.md"],
        now=100,
    )

    adapters = ReviewControlAdapters(
        state_reader=lambda payload: _review_state(findings=["missing bounded test"]),
        repair_enqueue=lambda task: repairs.append(task) or {"status": "DONE", "task": task["task_id"]},
    )
    first = run_scheduler_tick(
        scheduler_db_path=str(scheduler_db),
        loop_state_db_path=str(loop_db),
        now=100,
        review_adapters=adapters,
    )
    second = run_scheduler_tick(
        scheduler_db_path=str(scheduler_db),
        loop_state_db_path=str(loop_db),
        now=100,
        review_adapters=adapters,
    )

    store = SchedulerStore(scheduler_db)
    rereviews_before_done = [
        occurrence
        for schedule in store.list_enabled()
        for occurrence in store.list_occurrences(schedule.spec.schedule_id)
        if occurrence.proposal["payload"].get("repair_parent_reason")
    ]
    assert first["dispatch"]["done"] >= 1
    assert second["dispatch"]["done"] == 0
    assert len(repairs) == 1
    assert rereviews_before_done == []

    schedule_verified_repair_done(
        SchedulerStore(scheduler_db),
        repository="alanua/Skeleton",
        replacement_pr_number=2303,
        replacement_head_sha="c" * 40,
        source_issue=2301,
        allowed_files=["docs/AUTONOMOUS_REVIEW_GATE.md"],
        repair_task_id=str(repairs[0]["task_id"]),
        repair_idempotency_key=str(repairs[0]["idempotency_key"]),
        publication_status="DONE",
        now=101,
        reviewed_pr_number=2302,
        reviewed_head_sha=HEAD_SHA,
    )
    run_scheduler_tick(
        scheduler_db_path=str(scheduler_db),
        loop_state_db_path=str(loop_db),
        now=101,
        review_adapters=ReviewControlAdapters(
            state_reader=lambda payload: _review_state(),
            needs_operator_delivery=lambda packet: {"status": "DONE"},
        ),
    )
    rereviews_after_done = [
        occurrence
        for schedule in store.list_enabled()
        for occurrence in store.list_occurrences(schedule.spec.schedule_id)
        if occurrence.proposal["payload"].get("next_step") == "internal_review"
        and occurrence.proposal["payload"].get("pr_number") == 2303
    ]
    assert len(rereviews_after_done) == 1


def test_production_path_protected_pr_delivers_needs_operator_once(tmp_path) -> None:
    scheduler_db = tmp_path / "scheduler.sqlite3"
    loop_db = tmp_path / "loop.sqlite3"
    merges = []
    deliveries = []
    ensure_draft_pr_review_continuation(
        SchedulerStore(scheduler_db),
        repository="alanua/Skeleton",
        pr_number=2302,
        head_sha=HEAD_SHA,
        source_issue=2301,
        allowed_files=["scripts/runner_poll_github_tasks.py"],
        now=100,
    )
    adapters = ReviewControlAdapters(
        state_reader=lambda payload: _review_state(
            files=["scripts/runner_poll_github_tasks.py"]
        ),
        authorized_continuation=lambda packet: merges.append(packet) or {"status": "DONE"},
        needs_operator_delivery=lambda packet: deliveries.append(packet) or {"status": "DONE"},
    )

    run_scheduler_tick(
        scheduler_db_path=str(scheduler_db),
        loop_state_db_path=str(loop_db),
        now=100,
        review_adapters=adapters,
    )
    run_scheduler_tick(
        scheduler_db_path=str(scheduler_db),
        loop_state_db_path=str(loop_db),
        now=100,
        review_adapters=adapters,
    )

    store = SchedulerStore(scheduler_db)
    notification = store.get_operational_event(
        operator_notification_ledger_key(
            repository="alanua/Skeleton",
            pr_number=2302,
            head_sha=HEAD_SHA,
            reason="operator approval file changed: scripts/runner_poll_github_tasks.py",
        )
    )
    assert merges == []
    assert len(deliveries) == 1
    assert deliveries[0]["repository"] == "alanua/Skeleton"
    assert deliveries[0]["pr_number"] == 2302
    assert deliveries[0]["head_sha"] == HEAD_SHA
    assert deliveries[0]["permitted_merge_method"] == "squash"
    assert notification is not None


def test_production_path_stale_head_fails_closed_without_actions(tmp_path) -> None:
    scheduler_db = tmp_path / "scheduler.sqlite3"
    loop_db = tmp_path / "loop.sqlite3"
    repairs = []
    merges = []
    ensure_draft_pr_review_continuation(
        SchedulerStore(scheduler_db),
        repository="alanua/Skeleton",
        pr_number=2302,
        head_sha=HEAD_SHA,
        source_issue=2301,
        allowed_files=["docs/AUTONOMOUS_REVIEW_GATE.md"],
        now=100,
    )

    run_scheduler_tick(
        scheduler_db_path=str(scheduler_db),
        loop_state_db_path=str(loop_db),
        now=100,
        review_adapters=ReviewControlAdapters(
            state_reader=lambda payload: _review_state(head_sha="b" * 40),
            repair_enqueue=lambda packet: repairs.append(packet) or {"status": "DONE"},
            authorized_continuation=lambda packet: merges.append(packet) or {"status": "DONE"},
        ),
    )

    store = SchedulerStore(scheduler_db)
    do_not_merge = [
        occurrence
        for schedule in store.list_enabled()
        for occurrence in store.list_occurrences(schedule.spec.schedule_id)
        if occurrence.proposal["payload"].get("next_step") == "internal_repair_supersede_dependency"
    ]
    assert repairs == []
    assert merges == []
    assert len(do_not_merge) == 1
    assert do_not_merge[0].proposal["payload"]["review_verdict"] == "DO_NOT_MERGE"


def test_unknown_internal_control_action_fails_closed(tmp_path) -> None:
    scheduler_db = tmp_path / "scheduler.sqlite3"
    loop_db = tmp_path / "loop.sqlite3"
    store = SchedulerStore(scheduler_db)
    ensure_draft_pr_review_continuation(
        store,
        repository="alanua/Skeleton",
        pr_number=2302,
        head_sha=HEAD_SHA,
        source_issue=2301,
        allowed_files=["docs/AUTONOMOUS_REVIEW_GATE.md"],
        now=100,
    )
    schedule = store.list_enabled()[0]
    payload = dict(schedule.spec.payload)
    payload["next_step"] = "unknown"
    from core.review_gate import enqueue_internal_review_control

    enqueue_internal_review_control(store, payload=payload, now=101)

    receipt = run_scheduler_tick(
        scheduler_db_path=str(scheduler_db),
        loop_state_db_path=str(loop_db),
        now=101,
        review_adapters=ReviewControlAdapters(state_reader=lambda payload: _review_state()),
    )
    receipts = [
        dispatch
        for schedule in store.list_enabled()
        for occurrence in store.list_occurrences(schedule.spec.schedule_id)
        for dispatch in store.list_dispatch_receipts(occurrence.occurrence_id)
    ]

    assert receipt["dispatch"]["needs_operator"] >= 1
    assert any(item["reason"] == "UNKNOWN_INTERNAL_REVIEW_STEP" for item in receipts)
