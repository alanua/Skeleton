...
def test_direct_controller_path_uses_existing_transport_without_hmac_in_caller(
    ...
    monkeypatch.setattr(
        ...
        lambda request: (calls.append(request), executor_receipt(public_stdout()))[1],
    )
...
def test_altered_authority_rejected_before_transport(
    monkeypatch: pytest.MonkeyPatch,
    field: str,
    value: object,
) -> None:
...
def test_receipt_states_are_distinct() -> None:
