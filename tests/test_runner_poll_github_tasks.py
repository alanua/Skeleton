...
def _media_bootstrap_receipt() -> dict[str, object]:
    return {
        ...
        "final_postcheck_receipt_hash": "c" * 64,
...
def _post_migration_issue_body(expected_sha: str = HEAD_SHA) -> str:
    return "\n".join(
        (
            f"Mode: {runner.RUNTIME_MAINTENANCE_MODE}",
...
def _post_migration_receipt() -> dict[str, object]:
    return {
...
def test_home_edge_debian_media_bootstrap_is_allowlisted_and_dispatches_exact_receipt(
    ...
    assert "mutation_executor_receipt_hash=" + "b" * 64 in report
    assert "final_postcheck_receipt_hash=" + "c" * 64 in report
...
def test_home_edge_post_migration_reconcile_is_allowlisted_and_dispatches_sanitized_receipt(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    captured: dict[str, object] = {}
    ...
    def fake_execute(body: str, *, registered_clean_main_sha: str, github_main_sha: str):
        ...
        captured["registered_clean_main_sha"] = registered_clean_main_sha
        captured["github_main_sha"] = github_main_sha
        return _post_migration_receipt()
    ...
    assert "gallery_status=repaired_healthy" in report
    assert "watchdog_critical_count=0" in report
    assert "watchdog_warning_count=0" in report
...
def test_cross_project_expected_pr_zero_changes_blocks_and_keeps_worktree(
    tmp_path: Path,
) -> None:
    ...
    assert "observer_breadcrumb=provider_exit_0" in report
    assert "Issue workspace kept for review" in report
...
def test_cross_project_changed_worktree_publishes_before_cleanup(tmp_path: Path) -> None:
    ...
    publish_reports: list[str] = []

    def publish(body: str) -> str:
        publish_reports.append(body)
        assert "Target Project: lavalamp" in body
        assert "Target Repository: alanua/Lavalamp" in body
    ...
    report = post.call_args.args[1]
    assert report.startswith(runner._LOCAL_WORKTREE_DONE_PREFIX)
    assert "target_publication=done" in report
    assert "draft_pr_url=https://github.com/alanua/Lavalamp/pull/44" in report
    assert len(publish_reports) == 1
    cleanup.assert_called_once_with("alanua/Lavalamp", 13, "alanua/Lavalamp")
    labels.assert_any_call(13, runner.LABEL_RUNNING, runner.LABEL_DONE)
    notify.assert_called_once_with(13, "DONE", mock.ANY)
...
def test_cross_project_publication_failure_keeps_changed_worktree(
    ...
    with mock.patch.object(runner, "set_issue_label") as labels, mock.patch.object(
        ...
        return_value=(
            "BLOCKED: Runner host maintenance task failed.\n"
            "maintenance_task_id=publish_target_project_issue_worktree_pr\n"
    ...
    report = post.call_args.args[1]
    assert report.startswith("BLOCKED:")
    assert "target_publication=not_done" in report
...
def _codegen_update_existing_pr_issue_body(
    *,
    ...
    return "\n".join(
        (
            "Expected Output: update existing PR in place",
            "allowed_files:",
            *[f"  - {path}" for path in allowed_files],
            "",
            "```task",
            "operation: update_existing_pr",
            f"existing_pr: {pr_number}",
            f"expected_existing_pr_head_sha: {expected_head_sha}",
            "```",
        )
    )


def test_codegen_existing_pr_schema_alias_materializes_exact_existing_head() -> None:
    request, reason = runner._codegen_existing_pr_worktree_request(
        _codegen_update_existing_pr_issue_body()
    )

    assert reason is None
    assert request == runner.CodegenExistingPrWorktreeRequest(
        pr_number=2749,
...
def test_update_existing_pr_process_publishes_same_pr_and_queues_validation(
    ...
    issue = {
        ...
        "body": _codegen_update_existing_pr_issue_body(
            pr_number=2749,
            allowed_files=(
                "scripts/runner_poll_github_tasks.py",
                "tests/test_runner_poll_github_tasks.py",
    ...
    pre_pr_state = _pr_validation_state(
        number=2749,
    ...
    assert prepare.call_args.args[2] == runner.CodegenExistingPrWorktreeRequest(
        pr_number=2749,
        expected_head_sha=HEAD_SHA,
        expected_head_branch=None,
    )
...
def test_update_existing_pr_head_mismatch_blocks_before_publish_or_new_pr(
    ...
    issue = {
        "number": 2757,
        "title": "Refresh existing PR",
        "body": _codegen_update_existing_pr_issue_body(pr_number=2749),
        "comments": [],
        "labels": [runner.LABEL_READY],
    }
...
def _existing_pr_dirty_continuation_gate(
    *,
    issue_number: int = 1640,
...
def _prepare_dirty_existing_pr_worktree(
    tmp_path: Path,
) -> tuple[Path, Path, str, str]:
    seed, main_sha, head_sha = _prepare_existing_pr_head_seed(tmp_path)
    worktree_root = tmp_path / "worktrees"
    issue_path = worktree_root / "issue-1640"
    assert (
        runner.run_command(
            ["git", "worktree", "add", "-b", "runner/issue-1640", str(issue_path), head_sha],
            cwd=seed,
        )[0]
        == 0
    )
    assert (
        runner.run_command(
            ["git", "remote", "set-url", "origin", "https://github.com/alanua/Skeleton.git"],
            cwd=issue_path,
        )[0]
        == 0
    )
    (issue_path / "scripts" / "runner_poll_github_tasks.py").write_text(
        "retained dirty existing pr work\n", encoding="utf-8"
    )
    return seed, issue_path, main_sha, head_sha


def test_prepare_issue_worktree_from_existing_pr_head_retained_dirty_final_same_state_succeeds(
    tmp_path: Path,
) -> None:
    seed, issue_path, main_sha, head_sha = _prepare_dirty_existing_pr_worktree(tmp_path)
    pr_state = {
        "number": 1638,
        "state": "OPEN",
        "baseRefName": "main",
    ...
    with mock.patch.dict(
        os.environ, {"SKELETON_WORKTREE_ROOT": str(issue_path.parent)}, clear=True
    ), mock.patch.object(
        runner, "_get_pr_branch_validation_state", side_effect=[(pr_state, "gh"), (pr_state, "gh")]
    ) as pr_read:
        code, output, path = runner.prepare_issue_worktree_from_existing_pr_head(
            1640,
            seed,
            ...
            continuation_gate=_existing_pr_dirty_continuation_gate(),
        )

    assert code == 0, output
    assert path == issue_path
    assert pr_read.call_count == 2
    assert "retained_dirty_continuation_gate=allowed" in output
    assert "final_pr_metadata_source=gh" in output
    assert "$ git fetch origin" not in output
    assert "$ git checkout" not in output
    assert (
        issue_path / "scripts" / "runner_poll_github_tasks.py"
    ).read_text(encoding="utf-8") == "retained dirty existing pr work\n"


@pytest.mark.parametrize(
    ("final_updates", "reason"),
    (
        ({"headRefOid": "c" * 40}, "final_pr_head_sha_mismatch"),
        ({"state": "CLOSED"}, "final_pr_not_open"),
        ({"headRefName": "runner/issue-drift"}, "final_pr_head_branch_mismatch"),
        ({"number": 9999}, "final_pr_number_mismatch"),
    ),
)
def test_prepare_issue_worktree_from_existing_pr_head_retained_dirty_final_drift_blocks(
    tmp_path: Path,
    ...
    final_state = {**pr_state, **final_updates}
    ...
    with mock.patch.dict(
        ...
        runner,
        "_get_pr_branch_validation_state",
        side_effect=[(pr_state, "gh"), (final_state, "gh")],
    ) as pr_read:
        code, output, path = runner.prepare_issue_worktree_from_existing_pr_head(
            1640,
            seed,
...
def test_prepare_issue_worktree_from_existing_pr_head_retained_dirty_local_head_mismatch_blocks_before_final_read(
    tmp_path: Path,
) -> None:
    seed, issue_path, main_sha, head_sha = _prepare_dirty_existing_pr_worktree(tmp_path)
    _run_git(issue_path, "add", "scripts/runner_poll_github_tasks.py")
    _run_git(issue_path, "commit", "-m", "local retained head drift")
    (issue_path / "scripts" / "runner_poll_github_tasks.py").write_text(
        "retained dirty after local head drift\n", encoding="utf-8"
    )
    pr_state = {
        "number": 1638,
        "state": "OPEN",
    ...
    assert code == 1
    assert "reason=local_head_sha_mismatch" in output
...
def _universal_maintenance_issue_body(
    maintenance_task_id: str = runner.CHECK_SKELETON_FRESHNESS,
) -> str:
    privacy_boundary = (
        "PUBLIC_SAFE_AGGREGATE_ONLY"
        if maintenance_task_id == runner.PRIVATE_MEMORY_HEALTHCHECK
        else "PUBLIC_SAFE_REPOSITORY_ONLY"
    )
    publish_fields = (
        ("PR Number: 2162", f"Current Head SHA: {'c' * 40}")
        if maintenance_task_id in runner.PUBLISH_ONLY_MAINTENANCE_TASK_IDS
        else ()
    )
    return "\n".join(
        (
            f"Mode: {runner.RUNTIME_MAINTENANCE_MODE}",
            f"Maintenance Task ID: {maintenance_task_id}",
            *publish_fields,
            f"Base SHA: {'b' * 40}",
            "Allowed Files: scripts/runner_poll_github_tasks.py",
            "Approval Reference: EXPLICIT_PROTECTED_RUNNER_REPAIR_TEST_MERGE_RUNTIME_SYNC_20260715",
            "Idempotency Key: issue-1722-universal-maintenance-enforce",
            "Validation Timeout Seconds: 900",
            f"Privacy Boundary: {privacy_boundary}",
            "Expected Output: registry dispatch",
        )
    )
    ...
    *,
    ...
    for task_kind in compatibility.registered_task_kinds:
        ...
        task_handler = (
            ...
            else handler if task_kind == "code_edit" and handler else lambda _task: None
        )
        capabilities = (
...
def _vnext_node_snapshot(
    ...
    **updates: object,
    ...
    snapshot: dict[str, object] = {
        ...
        "node_id": runner.RUNNER_VNEXT_CODEGEN_NODE_ID,
        "generation": 7,
        "route_rank": runner.RUNNER_VNEXT_CODEGEN_ROUTE_RANK,
        "capabilities": list(runner.RUNNER_VNEXT_CODEGEN_CAPABILITIES),
        "supported_adapters": ["adapter:repo-codegen"],
        "supported_lanes": ["CODEGEN"],
        "privacy_classes": ["PUBLIC_SAFE"],
        "resource_patterns": [f"repo:{path}" for path in allowed_files],
        "observed_at": now - 1.0,
        "expires_at": now + 30.0,
        "attestation_ref": "attestation:runtime-public-safe-codegen-7",
    }
    snapshot.update(updates)
    return snapshot


def _set_vnext_node_snapshot(
    monkeypatch: pytest.MonkeyPatch,
    snapshot: object,
) -> None:
    monkeypatch.setenv(runner.RUNNER_VNEXT_NODE_SNAPSHOT_JSON_ENV, json.dumps(snapshot))
...
def test_authoritative_codegen_runs_mechanics_once_after_grant(
    ...
    _set_vnext_node_snapshot(monkeypatch, _vnext_node_snapshot(now=1_000.0))
    ...
    with mock.patch.object(
        ...
        with pytest.raises(runner.RunnerVNextAuthorityError) as exc:
            runner.run_authoritative_vnext_codegen(
                issue_number=3951,
                task_content="task prose attestation_ref:attestation:task-controlled",
                issue_workdir=str(workdir),
                metadata=metadata,
                legacy_runner_task=legacy_task,
            )
...
@pytest.mark.parametrize(
    ("maintenance_task_id", "expected_task_kind"),
    (
        (runner.CHECK_SKELETON_FRESHNESS, "repository_maintenance"),
        (runner.HOME_EDGE_01_READ_ONLY_DIAGNOSTIC, "diagnostic"),
        (runner.PRIVATE_MEMORY_HEALTHCHECK, "private_memory"),
        (runner.LOOP_ENGINE_PACKET, "loop_control"),
        (runner.INSPECT_ISSUE_WORKTREE_FOR_PUBLISH, "publish"),
    ),
)
def test_universal_runner_enforce_typed_maintenance_dispatches_registered_executor(
    tmp_path: Path,
    ...
    def typed_executor(task: object) -> str:
        dispatch_calls.append(task)
        return (
            "DONE: Runner host maintenance task completed.\n"
            f"maintenance_task_id={maintenance_task_id}\n"
            "success_criteria=met"
        )

    issue = {
        "number": 1722,
        "title": "Universal typed maintenance enforce",
        "body": _universal_maintenance_issue_body(maintenance_task_id),
        "comments": [
            _approval_comment(
                issue_number=1722,
    ...
    with mock.patch.dict(
        ...
        side_effect=lambda **_kwargs: _universal_registry(
            handlers_by_kind={expected_task_kind: typed_executor}
        ),
    ) as registry, mock.patch.object(
        runner, "dispatch_runtime_maintenance_task"
    ) as legacy_dispatch, mock.patch.object(
    ...
    assert len(dispatch_calls) == 1
    dispatched_task = dispatch_calls[0]
    assert dispatched_task.task_kind == expected_task_kind
    assert dispatched_task.payload["maintenance_task_id"] == maintenance_task_id
...
def test_universal_runner_enforce_executor_exception_fails_closed_without_legacy_side_effect(
    ...
    issue = {
        "number": 1722,
        "title": "Universal repository maintenance enforce failure",
    ...
    with mock.patch.dict(
        ...
        runner, "record_runner_executor_result"
    ) as record:
        runner.process_issue(issue, workdir=str(tmp_path))

    assert "Universal Runner executor dispatch failed: EXECUTOR_RAISED." in block.call_args.args[1]
    legacy_dispatch.assert_not_called()
    prepare.assert_not_called()
    codex.assert_not_called()
...
def test_recover_skeleton_checkout_dirty_runner_branch_preserves_before_main_transition(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _origin, _seed, checkout_path = _prepare_recover_skeleton_checkout_repo(tmp_path)
    recovery_root = tmp_path / "recovery"
    monkeypatch.setenv("SKELETON_RUNNER_CHECKOUT_RECOVERY_ROOT", str(recovery_root))
    assert runner.run_command(["git", "checkout", "-b", "runner/issue-2096"], cwd=checkout_path)[
        0
    ] == 0
    tracked = checkout_path / "tracked.txt"
    untracked = checkout_path / "new.txt"
    tracked.write_text("base\nmodified\n", encoding="utf-8")
    untracked.write_text("untracked\n", encoding="utf-8")
    origin_main_sha = runner.run_command(
        ["git", "rev-parse", "origin/main"], cwd=checkout_path
    )[1].strip()
    project_tree = _project_tree_for_skeleton_checkout(checkout_path)
    ...
    with mock.patch.object(
        ...
        report = runner.recover_skeleton_checkout(
            _recover_skeleton_checkout_issue_body(expected_head_sha=origin_main_sha)
        )

    assert report.startswith("DONE:")
    assert "current_branch=runner/issue-2096" in report
    assert "recovery_stash_status=created" in report
    assert "recovery_artifact_status=verified" in report
    assert "step=checkout_main status=done" in report
    assert "reset_status=hard_origin_main" in report
    ...
    assert "checkout_sync_state=equal" in report
    assert str(recovery_root) not in report
    assert "tracked.txt" not in report
    assert "new.txt" not in report
    assert runner.run_command(["git", "symbolic-ref", "--short", "HEAD"], cwd=checkout_path)[
        1
    ].strip() == "main"
    assert runner.run_command(["git", "rev-parse", "HEAD"], cwd=checkout_path)[
        1
    ].strip() == origin_main_sha
    assert runner.run_command(
        ["git", "status", "--porcelain", "--untracked-files=all"], cwd=checkout_path
    )[1].strip() == ""
...
def test_recover_skeleton_checkout_clean_runner_branch_transitions_without_recovery(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _origin, _seed, checkout_path = _prepare_recover_skeleton_checkout_repo(tmp_path)
    recovery_root = tmp_path / "recovery"
    ...
    assert report.startswith("DONE:")
    assert "current_branch=runner/issue-2096" in report
    assert "recovery_stash_status=not_needed" in report
    assert "recovery_artifact_status=verified" not in report
    assert "step=checkout_main status=done" in report
...
def test_recover_skeleton_checkout_feature_preservation_failure_leaves_state_untouched(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _origin, _seed, checkout_path = _prepare_recover_skeleton_checkout_repo(tmp_path)
    recovery_root = tmp_path / "recovery"
    ...
    assert report.startswith("NEEDS_OPERATOR:")
    assert "reason=recovery_artifact_verify_failed" in report
    assert "step=checkout_main status=done" not in report
    assert runner.run_command(["git", "symbolic-ref", "--short", "HEAD"], cwd=checkout_path)[
        1
    ].strip() == "runner/issue-2096"
    assert tracked.read_text(encoding="utf-8") == "base\nmodified\n"
    assert untracked.read_text(encoding="utf-8") == "untracked\n"
...
@pytest.mark.parametrize(
    ...
    [
        ("", 1, "reason=detached_head"),
        ("feature/recover\n", 0, "reason=branch_not_recoverable"),
        ("runner/issue-0\n", 0, "reason=branch_not_recoverable"),
        ("runner/issue-2096-extra\n", 0, "reason=branch_not_recoverable"),
    ],
)
def test_recover_skeleton_checkout_rejects_detached_and_non_runner_branches_before_mutation(
    branch_output: str, branch_code: int, reason: str
) -> None:
    checkout_path = _safe_checkout_path("skeleton-recover-invalid-branch")
    project_tree = _project_tree_for_skeleton_checkout(checkout_path)
    ...
    with mock.patch.object(
        ...
        runner, "_create_verified_skeleton_checkout_recovery"
    ) as preserve:
        path_exists.side_effect = lambda path: path in {
            checkout_path,
            checkout_path / ".git",
    ...
    assert not any(command[2:3] == ["push"] for command in commands)
    assert ["git", "-C", str(checkout_path), "stash", "push"] not in commands
    assert ["git", "-C", str(checkout_path), "reset", "--hard", "origin/main"] not in commands
    assert ["git", "-C", str(checkout_path), "clean", "-fd"] not in commands
    preserve.assert_not_called()


def test_recover_skeleton_checkout_expected_sha_mismatch_blocks_before_mutation() -> None:
    checkout_path = _safe_checkout_path("skeleton-recover-expected-mismatch")
    project_tree = _project_tree_for_skeleton_checkout(checkout_path)
    origin_main_sha = "b" * 40
    expected_head_sha = "c" * 40
    ...
    def run_recover_command(
        ...
        if command == ["git", "-C", str(checkout_path), "remote", "get-url", "origin"]:
            return 0, "https://github.com/alanua/Skeleton.git\n"
        if command == ["git", "-C", str(checkout_path), "symbolic-ref", "--short", "HEAD"]:
            return 0, "runner/issue-2096\n"
        if command == ["git", "-C", str(checkout_path), "status", "--porcelain"]:
            return 0, " M tracked.txt\n?? new.txt\n"
        if command == ["git", "-C", str(checkout_path), "fetch", "--prune", "origin", "main"]:
            return 0, "fetch output must not leak"
        if command == ["git", "-C", str(checkout_path), "rev-parse", "origin/main"]:
            return 0, f"{origin_main_sha}\n"
        return 2, "unexpected command"
    ...
    with mock.patch.object(
        ...
        report = runner.recover_skeleton_checkout(
            _recover_skeleton_checkout_issue_body(expected_head_sha=expected_head_sha)
        )
...
def test_recover_skeleton_checkout_feature_branch_command_ordering_and_bounded_inputs() -> None:
    checkout_path = _safe_checkout_path("skeleton-recover-ordering")
    project_tree = _project_tree_for_skeleton_checkout(checkout_path)
    origin_main_sha = "b" * 40
    events: list[object] = []
    ...
    def run_recover_command(
        ...
        if command == ["git", "-C", str(checkout_path), "remote", "get-url", "origin"]:
            return 0, "https://github.com/alanua/Skeleton.git\n"
        if command == ["git", "-C", str(checkout_path), "symbolic-ref", "--short", "HEAD"]:
            if run_recover_command.on_main:
                return 0, "main\n"
            return 0, "runner/issue-2096\n"
        if command == ["git", "-C", str(checkout_path), "status", "--porcelain"]:
            return 0, " M tracked.txt\n?? new.txt\n"
        if command == ["git", "-C", str(checkout_path), "fetch", "--prune", "origin", "main"]:
            return 0, "raw fetch output must not leak"
        if command == ["git", "-C", str(checkout_path), "rev-parse", "origin/main"]:
            return 0, f"{origin_main_sha}\n"
        if command == ["git", "-C", str(checkout_path), "checkout", "-B", "main", "origin/main"]:
            run_recover_command.on_main = True
            return 0, "checkout output must not leak"
        if command == ["git", "-C", str(checkout_path), "reset", "--hard", "origin/main"]:
            return 0, "reset output must not leak"
        if command == ["git", "-C", str(checkout_path), "clean", "-fd"]:
            return 0, "clean output must not leak"
        if command == [
            "git",
            "-C",
            str(checkout_path),
    ...
    run_recover_command.on_main = False

    def preserve(
        path: Path, status_lines: list[str]
    ) -> tuple[str | None, str | None]:
        events.append("verified_preservation")
        assert path == checkout_path
        status_lines.extend(
            (
                "recovery_stash_status=created",
                "recovery_ref_status=created",
                "recovery_artifact_status=verified",
            )
        )
        return "a" * 40, None
    ...
    with mock.patch.object(
        ...
        runner, "_create_verified_skeleton_checkout_recovery", side_effect=preserve
    ):
        path_exists.side_effect = lambda path: path in {
            checkout_path,
            checkout_path / ".git",
        ...
        report = runner.recover_skeleton_checkout(
            _recover_skeleton_checkout_issue_body(
                expected_head_sha=origin_main_sha,
                task_body="git push origin evil\n"
                "gh pr merge 1\n"
                "git checkout -B main issue-controlled",
            )
        )
    ...
    assert events == [
        ...
        ["git", "-C", str(checkout_path), "clean", "-fd"],
        [
            "git",
            "-C",
            str(checkout_path),
...
def test_validate_pr_branch_runner_exact_base_route_reports_safe_command_exception_token(
    ...
    def run_validation_command(
        ...
        if command == ["git", "rev-parse", "HEAD"] and cwd == validation_path:
            return 0, f"{HEAD_SHA}\n"
        if command == ["git", "status", "--short"] and cwd == validation_path:
            return 0, ""
        return 2, "unexpected command"
    ...
    assert "validation_profile=runner_exact_base" in report
    assert "step=validation_profile_command_1 status=failed exit_code=127" in report
    assert (
        "validation_command_text=python3_-m_pytest_-q_tests/test_runner_poll_github_tasks.py"
        in report
    )
...
def test_run_codex_task_sanitizes_home_edge_environment(
    ...
    assert "live-home-edge" not in output
    assert SYNTHETIC_HOME_EDGE_EXEC_HMAC not in output
    assert "/private/runner.sqlite" not in output
...
def _gmail_primary_activation_body(
    *,
    expected_main_sha: str = HEAD_SHA,
    account_alias: str = "acct:gmail-primary",
    approval: str = "EXACT_HEAD_OPERATOR_REVIEW_THEN_RUNTIME_SYNC_APPROVED",
) -> str:
    return "\n".join(
        (
            f"Mode: {runner.RUNTIME_MAINTENANCE_MODE}",
            f"Maintenance Task ID: {runner.MAIL_GMAIL_PRIMARY_REGISTERED_ACTIVATION}",
            f"Repository: {runner.REPO}",
            f"Expected Main SHA: {expected_main_sha}",
            f"Account Alias: {account_alias}",
...
def test_gmail_primary_activation_blocks_before_reference_without_sync_approval() -> None:
    with (
        ...
        report = runner.dispatch_runtime_maintenance_task(
            runner.MAIL_GMAIL_PRIMARY_REGISTERED_ACTIVATION,
            "/synthetic",
            _gmail_primary_activation_body(approval="PENDING"),
        )
...
def test_gmail_primary_activation_blocks_on_reference_bootstrap_before_canary() -> None:
    ...
    with (
        ...
        mock.patch.object(
            runner,
            "_bootstrap_mail_gmail_primary_reference_index",
            side_effect=runner.SecretReferenceRegistrationError("REFERENCE_BOOTSTRAP_REQUIRED"),
        ) as bootstrap,
        mock.patch.object(runner, "run_gmail_readonly_canary") as canary,
        mock.patch.object(runner, "run_command") as command,
...
def test_gmail_primary_activation_bootstraps_rereads_then_canary_and_timer(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    preflight_patch, sha_patch = _gmail_readonly_preflight_patches(HEAD_SHA)
    ...
    def fake_reference(authority_environment, *, service_id, alias, bootstrap_required):
        del bootstrap_required
        reference_calls.append(f"{service_id}:{alias}")
        path = Path(authority_environment["CREDENTIALS_DIRECTORY"]) / "skeleton-secret-reference-index"
        if not path.exists():
            raise runner.SecretReferenceRegistrationError("REFERENCE_BOOTSTRAP_REQUIRED")
        return SecretReference(provider="bitwarden", reference_id=secret_id)

    def fake_command(command, cwd=None, **kwargs):
        del cwd
        calls.append(" ".join(command))
        if command[:4] == ["sudo", "-n", "systemd-creds", "encrypt"]:
            assert kwargs["input"].startswith('{"registrations"')
            assert "11111111-2222-3333-4444-555555555555" in kwargs["input"]
            return 0, ""
        if command == runner._bitwarden_gmail_reference_helper_command():
            return 0, json.dumps(
                {
                    "status": "DONE",
                    "index": {
    ...
    assert runner.maintenance_report_status(report) == "DONE"
    assert "step=reference_bootstrap status=done" in report
    assert "step=reference_bind status=done" in report
    assert bind.call_count == 2
    assert reference_calls == ["mail-gmail:acct:gmail-primary", "mail-gmail:acct:gmail-primary"]
    assert canary.call_count == 1
    assert calls == [
        " ".join(runner._bitwarden_gmail_reference_helper_command()),
        (
            "sudo -n systemd-creds encrypt "
