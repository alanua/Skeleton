...
def _clear_notification_idempotency_keys() -> None:
    runner._NOTIFICATION_IDEMPOTENCY_KEYS.clear()
...
def test_done_pr_notification_is_suppressed_before_card_build() -> None:
    ...
    ) as keyboard, mock.patch.object(runner.urllib.request, "urlopen") as urlopen:
...
def test_done_notification_does_not_fetch_issue_or_send() -> None:
    issue = {
        "number": 129,
    ...
    ) as get_issue, mock.patch.object(runner.urllib.request, "urlopen") as urlopen:
        runner.notify_task_finished(129, "DONE", DONE_REPORT)
...
def test_cross_project_blocked_notification_is_suppressed() -> None:
    issue = {
        "number": 999,
        "body": (
...
def test_needs_operator_uses_target_repository_and_emits_once() -> None:
    issue = {
        "number": 999,
        "body": (
            "Target Project: lumenflow\n"
    ...
    with mock.patch.object(
        ...
        runner.notify_task_finished(999, "NEEDS_OPERATOR", "NEEDS_OPERATOR: inspect")
        runner.notify_task_finished(999, "NEEDS_OPERATOR", "NEEDS_OPERATOR: inspect")
...
def test_done_pr_card_build_failure_is_not_reached_for_suppressed_done() -> None:
    with mock.patch.object(
        runner,
...
def test_done_pr_reply_markup_send_failure_is_not_reached_for_suppressed_done() -> None:
    card = {"text": "PR ready card", "buttons": []}
    reply_markup = {"inline_keyboard": []}
