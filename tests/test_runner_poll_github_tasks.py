...
def _assert_skeleton_public_report_omits_checkout_path(
    report: str, checkout_path: Path
) -> None:
    assert "target_project=skeleton" in report
    assert f"target_repository={runner.REPO}" in report
...
def test_check_skeleton_freshness_reports_done_with_bounded_status_queries() -> None:
    ...
    _assert_skeleton_public_report_omits_checkout_path(report, checkout_path)
...
def test_check_skeleton_freshness_unsafe_path_blocks_before_commands() -> None:
    ...
    assert report.startswith("BLOCKED:")
    _assert_skeleton_public_report_omits_checkout_path(report, Path("/tmp/Skeleton"))
    assert "reason=checkout_path_unsafe" in report
    run.assert_not_called()
