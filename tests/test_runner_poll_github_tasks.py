...
def test_home_edge_display_power_off_is_allowlisted_and_dispatches_exact_task_id(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    captured: dict[str, object] = {}
    ...
    assert "physical_observation=off" in report
    assert "physical_verification_reason=dpms_monitor_off" in report
    assert "success_criteria=met" in report
    assert captured["registered_clean_main_sha"] == HEAD_SHA
    assert captured["github_main_sha"] == HEAD_SHA
    assert captured["body"] == _display_power_off_issue_body()
    assert "EXPLICIT_2026_08_09_TURN_OFF_HOME_EDGE_MONITOR" not in report
...
def test_home_edge_display_power_off_registry_exposes_only_exact_fixed_task_id() -> None:
    variants = {
        task_id
        for task_id in runner.RUNTIME_MAINTENANCE_TASK_IDS
        if "display_power_off" in task_id or "display-off" in task_id
    }
