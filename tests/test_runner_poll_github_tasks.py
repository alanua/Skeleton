...
def _display_power_off_issue_body(expected_sha: str = HEAD_SHA, **updates: str) -> str:
    fields = {
...
def _display_power_off_receipt(**updates: object) -> dict[str, object]:
    receipt: dict[str, object] = {
        "maintenance_task_id": runner.HOME_EDGE_01_DISPLAY_POWER_OFF_V1,
        "operation_id": runner.HOME_EDGE_01_DISPLAY_POWER_OFF_V1,
        "request_accepted": True,
        "applied": True,
        "physically_verified": "unobservable",
        "mutation_executor_receipt_hash": "d" * 64,
...
def test_home_edge_display_power_off_is_allowlisted_and_dispatches_sanitized_receipt(
    ...
    assert "operation_id=home_edge_01_display_power_off_v1" in report
    assert "request_accepted=True" in report
    assert "applied=True" in report
    assert "physically_verified=unobservable" in report
    assert "success_criteria=met" in report
    assert captured["registered_clean_main_sha"] == HEAD_SHA
    assert captured["github_main_sha"] == HEAD_SHA
    assert "/home/" not in report
    assert "DISPLAY" not in report
    assert "SECRET" not in report
...
def test_home_edge_display_power_off_missing_approval_blocks(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(runner, "_read_exact_git_sha", lambda _ref: HEAD_SHA)
    ...
    report = runner.dispatch_runtime_maintenance_task(
        runner.HOME_EDGE_01_DISPLAY_POWER_OFF_V1,
        str(runner.ROOT),
        _display_power_off_issue_body(**{"Operator Approval": ""}),
    )
