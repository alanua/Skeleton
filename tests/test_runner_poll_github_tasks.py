...
def _graphify_platform_skill(tmp_path: Path, platform: str) -> Path:
    return tmp_path / runner.GRAPHIFY_UPSTREAM_PLATFORM_SKILL_RELATIVE_PATHS[platform]
...
def test_graphify_runtime_restores_third_platform_version_stamp_after_later_failure(
    ...
    third_skill = _graphify_platform_skill(home, "claude")
    third_skill.parent.mkdir(parents=True)
    third_skill.write_text("existing claude\n", encoding="utf-8")
    third_marker = _graphify_platform_version_marker(home, "claude")
