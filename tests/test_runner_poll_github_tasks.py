...
def _display_power_off_issue_body(
...
def _display_power_off_receipt(**overrides: object) -> dict[str, object]:
...
def test_home_edge_display_power_off_is_allowlisted_and_dispatches_exact_receipt(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    captured: dict[str, object] = {}
    ...
    assert "display_power_status=off" in report
    assert "idempotency_status=executed" in report
    assert "executor_receipt_hash=" + "b" * 64 in report
    assert "success_criteria=met" in report
    assert captured["registered_clean_main_sha"] == HEAD_SHA
    assert captured["github_main_sha"] == HEAD_SHA
...
def test_home_edge_display_power_off_requires_exact_operator_approval(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(runner, "_read_exact_git_sha", lambda _ref: HEAD_SHA)
    ...
    report = runner.dispatch_runtime_maintenance_task(
        runner.HOME_EDGE_01_DISPLAY_POWER_OFF_V1,
        str(runner.ROOT),
        _display_power_off_issue_body(approval="EXPLICIT_TURN_OFF_MONITOR"),
    )

    assert report.startswith("BLOCKED:")
    assert "reason=operator_approval_mismatch" in report
