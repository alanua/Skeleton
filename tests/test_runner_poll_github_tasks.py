...
def _issue_publish_commands(
    *,
    ...
    def run(command: list[str], cwd: str | Path | None = None) -> tuple[int, str]:
        ...
        if command == ["git", "diff", "--cached", "--check", "--", *changed_files]:
            return staged_diff_check_code, "diff check output must not leak"
        if command == [
            "git",
            "commit",
            "-m",
...
def test_publish_issue_worktree_pr_staged_diff_check_failure_blocks_before_commit(
    tmp_path: Path,
) -> None:
    worktree_path = _prepare_issue_publish_worktree(tmp_path)
    with mock.patch.object(
        ...
        side_effect=_issue_publish_commands(
            worktree_path=worktree_path,
            staged_diff_check_code=1,
        ),
    ) as run:
        report = runner.publish_issue_worktree_pr(_issue_publish_body())

    commands = [call.args[0] for call in run.call_args_list]
    add_command = ["git", "add", "--", "scripts/runner_poll_github_tasks.py"]
    ...
    assert report.startswith("BLOCKED:")
    assert "reason=staged_diff_check_failed" in report
    assert commands.index(add_command) < commands.index(staged_check_command)
    assert all(command[:2] != ["git", "commit"] for command in commands)
    assert all(command[:2] != ["git", "push"] for command in commands)
