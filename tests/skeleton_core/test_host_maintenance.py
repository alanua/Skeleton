...
def test_windows_bootstrap_one_link_writes_private_artifact_and_redacted_public_report(tmp_path: Path) -> None:
    ...
    assert action["token_lifecycle"] == "no_automatic_expiry_manual_rotation_or_successful_enrollment_only"
    ...
    assert "expires_at" not in public_blob
    assert "EXPIRED" not in public_blob
    assert "ttl" not in public_blob.lower()
    ...
    assert private_payload["delivery_channel"] == "viber_https_link"
    assert private_payload["token_lifecycle"] == "no_automatic_expiry_manual_rotation_or_successful_enrollment_only"
    assert "expires_at" not in private_payload
    assert private_payload["endpoint_payload_schema"] == "skeleton.home_edge.remote_windows_audit.owner_enrollment.v1"
