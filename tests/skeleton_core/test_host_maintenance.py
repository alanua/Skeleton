...
def test_windows_bootstrap_one_link_writes_private_artifact_and_redacted_public_report(tmp_path: Path) -> None:
    ...
    assert "code=" not in public_blob
    ...
    assert private_payload["status"] == "ISSUED"
    assert private_payload["token_status"] == "ISSUED"
    assert private_payload["single_use"] is True
    assert private_payload["use_limit"] == 1
    assert private_payload["use_count"] == 0
    assert private_payload["automatic_expiry"] is False
    assert private_payload["delivery_channel"] == "owner_viber_private_https"
    assert private_payload["expires_at"] is None
    assert private_payload["consumed_at"] is None
    assert private_payload["revoked_at"] is None
