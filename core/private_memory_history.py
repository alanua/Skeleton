...
_REQUIRED_TABLES = {
    "private_memory_meta": {"key", "value"},
    "private_memory_canonical_revision": {"id", "schema", "current_revision", "updated_at"},
...
def validate_history_schema(connection: sqlite3.Connection) -> None:
    ...
    for table_name, required_columns in _REQUIRED_TABLES.items():
        columns = {str(row[1]) for row in connection.execute(f"PRAGMA table_info({table_name})")}
        if not required_columns.issubset(columns):
            raise PrivateMemoryIntegrityFailure("incomplete canonical schema table")
...
def _history_chain_ok(connection: sqlite3.Connection) -> bool:
    ...
    if fact_keys != set(latest_history_by_fact):
        return False
