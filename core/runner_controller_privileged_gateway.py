...
MAX_REPLAY_LEDGER_BYTES: Final = 1024 * 1024
MAX_REPLAY_LEDGER_ENTRIES: Final = 4096
MAX_SUBPROCESS_OUTPUT_BYTES: Final = 64 * 1024
...
_SSH_PUBLIC_KEY_RE = re.compile(
    r"^(ssh-ed25519|ecdsa-sha2-nistp256) ([A-Za-z0-9+/]+={0,2})(?: [A-Za-z0-9_.@:+/=-]{1,80})?$"
)


def validate_approved_ssh_public_key(public_key: str) -> str:
    if not isinstance(public_key, str):
        raise PrivilegedGatewayError("ssh_public_key_invalid")
    if not public_key or "\r" in public_key or "\n" in public_key:
        raise PrivilegedGatewayError("ssh_public_key_invalid")
    if len(public_key.encode("utf-8")) > 512:
        raise PrivilegedGatewayError("ssh_public_key_too_large")
    if public_key != public_key.strip():
        raise PrivilegedGatewayError("ssh_public_key_invalid")
    match = _SSH_PUBLIC_KEY_RE.fullmatch(public_key)
    if match is None:
        raise PrivilegedGatewayError("ssh_public_key_invalid")
    algorithm, key_body = match.group(1), match.group(2)
    import base64
    import binascii

    try:
        decoded = base64.b64decode(key_body.encode("ascii"), validate=True)
    except (binascii.Error, UnicodeEncodeError) as exc:
        raise PrivilegedGatewayError("ssh_public_key_base64_invalid") from exc
    if len(decoded) < 32:
        raise PrivilegedGatewayError("ssh_public_key_malformed")
    expected_prefix = len(algorithm).to_bytes(4, "big") + algorithm.encode("ascii")
    if not decoded.startswith(expected_prefix):
        raise PrivilegedGatewayError("ssh_public_key_algorithm_mismatch")
    return public_key
...
def _read_ledger_entries(path: Path) -> list[Mapping[str, object]]:
    try:
        st = path.stat()
        if st.st_size > MAX_REPLAY_LEDGER_BYTES:
            raise PrivilegedGatewayError("replay_ledger_too_large")
        if stat.S_IMODE(st.st_mode) & 0o077:
            raise PrivilegedGatewayError("replay_ledger_writable")
    ...
    for line in data.splitlines():
        ...
        if len(entries) > MAX_REPLAY_LEDGER_ENTRIES:
            raise PrivilegedGatewayError("replay_ledger_entry_limit")
    return entries
    ...
    *,
    ...
    try:
        ...
        if stat.S_IMODE(parent_stat.st_mode) & 0o077:
            raise PrivilegedGatewayError("replay_ledger_parent_writable")
    except OSError as exc:
        raise PrivilegedGatewayError("replay_ledger_unavailable") from exc
    try:
        os.mkdir(lock, 0o700)
    ...
    try:
        ...
        try:
            ...
            current_size = 0
        if current_size + len(line) > MAX_REPLAY_LEDGER_BYTES:
            raise PrivilegedGatewayError("replay_ledger_too_large")
...
def _root_local_protected_run(argv: list[str], timeout: int | None) -> tuple[int, str]:
    ...
    child_argv = argv[2:]
    install_dest = str(HOME_EDGE_ESP_LAB_STAGE1_SIGNER_PROTECTED_INSTALLER)
    install_prefix = [
        "/usr/bin/install",
        "-D",
        "-o",
    ...
    install_allowed = (
        len(child_argv) == len(install_prefix) + 2
        and child_argv[: len(install_prefix)] == install_prefix
        and child_argv[-1] == install_dest
        and child_argv[-2].startswith("/tmp/skeleton-esp-stage1-signer-")
    )
    if not install_allowed and child_argv != signer_argv:
        raise PrivilegedGatewayError("nested_privilege_argv_not_allowed")
    ...
    if len(output.encode("utf-8", "replace")) > MAX_SUBPROCESS_OUTPUT_BYTES:
        raise PrivilegedGatewayError("nested_privilege_output_too_large")
...
class SshForcedCommandGatewayTransport:
    ...
    def authorized_keys_line(self, public_key: str) -> str:
        key = validate_approved_ssh_public_key(public_key)
...
def deterministic_sshd_config_fragment() -> str:
    return "\n".join(
        (
            ...
            "    PermitListen none",
            "    PermitUserRC no",
            "    AuthorizedKeysFile /var/lib/skeleton/runner-controller/ssh/authorized_keys",
