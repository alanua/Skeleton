...
def compile_repository_codegen_context(
    ...
    *,
    ...
    _public_ref(adapter_id, "REPOSITORY_CODEGEN_ADAPTER_ID_INVALID")
    ...
    if not {"repository_read", "repository_write_allowlisted", "test_execution"}.issubset(required_capabilities):
    ...
    binding_hash = repository_codegen_binding_hash(
    ...
    task_id = f"task:repository-codegen:{binding_hash}"
    operation_id = f"op:repository-codegen:{binding_hash}"
    idempotency_key = f"idem:repository-codegen:{binding_hash}"
    evidence = (
        source_task_ref,
        adapter_id,
        f"runner-task-sha256:{runner_task_hash}",
        f"repository-codegen-binding-sha256:{binding_hash}",
        target_state_ref,
    )
    universal_task = UniversalTask(
        task_id=task_id,
...
def repository_codegen_binding_hash(
    *,
    runner_task_json_sha256: str,
    source_task_ref: str,
    adapter_id: str,
    ...
    payload = {
        "adapter_id": adapter_id,
        "runner_task_json_sha256": runner_task_json_sha256,
        "source_task_ref": source_task_ref,
    }
...
def _validate_grant_binding(context: RepositoryCodegenContext, grant: GreenExecutionGrant) -> None:
    ...
    if context.binding_hash != repository_codegen_binding_hash(
        runner_task_json_sha256=context.runner_task_json_sha256,
        source_task_ref=context.source_task_ref,
        adapter_id=context.adapter_id,
    ):
        raise GreenExecutionError("REPOSITORY_CODEGEN_BINDING_HASH_MISMATCH")
    expected = (
        context.task_id,
    ...
    if grant.planner_envelope.required_capabilities != context.runner_task.requested_capabilities:
        raise GreenExecutionError("REPOSITORY_CODEGEN_GRANT_CAPABILITY_MISMATCH")
