...
def build_email_message_prompt(payload: Mapping[str, Any]) -> str:
    ...
    return (
        ...
        "Calibrate with these examples: Bitwarden/Google/Hetzner verification code from the legitimate service sender => security, is_spam_suspected=false. Samsung/Kaufland/Wizz discount offer => ads, low/normal importance. Google Flights tracked-route price alert => travel. GitHub workflow 'No jobs were run' with no consequence => technical, technical_consequence=false, low importance. A fake parcel/prize/bank-login lure impersonating a brand => spam and possibly security. "
