...
class JsonCommandAdapter:
    ...
    def execute(self, payload: Mapping[str, Any]) -> AdapterResult:
        ...
        if not isinstance(status, str):
            raise SinkError("adapter_response_invalid")
        return AdapterResult(status=status, payload=dict(response))
    ...
    def _run(command: Sequence[str], input_text: str, timeout: int, max_output: int) -> tuple[int, str, str]:
        try:
            ...
            return 1, "", ""
        if len(completed.stdout.encode("utf-8")) + len(completed.stderr.encode("utf-8")) > max_output:
            return 1, "", ""
...
class MemoryGatewaySink:
    ...
    def commit_and_readback(
        ...
        *,
        ...
        read_request = build_exact_read(payload["fact_namespace"], payload["fact_id"])
        read_result = self.adapter.execute(read_request)
        ...
        if isinstance(value, Mapping) and value.get("document_id") not in {None, record.get("document_id")}:
...
class CalendarSink:
    ...
    def upsert(self, event: Mapping[str, Any]) -> str:
        ...
        request = {
            "schema": "skeleton.family_document.calendar_upsert.v1",
            "operation": "upsert",
...
def build_private_mutation(
    ...
    *,
    ...
    source_hash: str,
    expected_revision: int | None = None,
    actor_ref: str = "skeleton.family_document_intake",
    reason_code: str = "family_document_record_commit",
) -> dict[str, Any]:
    ...
    fact_digest = _digest(document_id, str(record.get("record_revision", "v1")))
    fact_id = f"document:{fact_digest[:48]}"
...
def build_exact_read(fact_namespace: str, fact_id: str) -> dict[str, Any]:
    namespace = _safe_token(fact_namespace, "fact_namespace")
    identifier = _safe_token(fact_id, "fact_id")
    request = {
        "schema": GATEWAY_REQUEST_SCHEMA,
        "namespace": NAMESPACE,
        ...
        "payload": {
            "project_id": NAMESPACE,
            "dataset_id": DATASET_ID,
            "canonical_ref": f"{namespace}:{identifier}",
        },
    }
...
def _safe_token(value: str, field_name: str) -> str:
    ...
    if any(marker in value.casefold() for marker in ("secret", "password", "credential", "/", "\\")):
        raise SinkError(f"{field_name}_invalid")
    return value
