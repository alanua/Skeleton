...
_INTERNAL_REVIEW_RE = re.compile(
    r"internal_review_verdict=(APPROVE|REQUEST_CHANGES|DO_NOT_MERGE)"
)
...
def routine_review_outcome(report: str | None) -> bool:
    text = report or ""
    return bool(_INTERNAL_REVIEW_RE.search(text) or _PR_READY_RE.search(text))
