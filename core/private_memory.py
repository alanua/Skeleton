...
def initialize_private_memory_schema(
    config_path: str | Path | None = None,
    *,
    env: Mapping[str, str] | None = None,
) -> dict[str, object]:
    """Explicitly initialize the public-safe connector heartbeat schema."""
    connector = PrivateMemoryConnector(config_path=config_path, env=env)
    return connector.initialize_schema()
...
class CanonicalPrivateMemoryStore:
    ...
    def initialize(self) -> dict[str, object]:
        with closing(self._connect(write=True, create=True)) as connection:
    ...
    def _connect(self, *, write: bool, create: bool = False) -> sqlite3.Connection:
        if write:
            if create:
                ...
                connection = sqlite3.connect(f"file:{self.db_path.as_posix()}?mode=rw", uri=True)
        else:
            if not self.db_path.is_file():
                raise PrivateMemoryConfigError("database not found")
...
class PrivateMemoryConnector:
    ...
    def initialize_schema(self) -> dict[str, object]:
        try:
            db_path = self._load_db_path()
            with closing(self._connect(db_path, write=True)) as connection:
                self._ensure_schema(connection)
    ...
    def _require_schema(self, connection: sqlite3.Connection) -> None:
        ...
        if not schema_present:
            raise PrivateMemorySchemaError("connector schema unavailable")
