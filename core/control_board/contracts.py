...
class ControlBoardSnapshot:
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> ControlBoardSnapshot:
        ...
        _reject_unknown_fields(value, SNAPSHOT_FIELDS, "snapshot")
...
def _metric(value: Mapping[str, Any]) -> Metric:
    _reject_unknown_fields(value, {"label", "value", "tone"}, "metrics item")
...
def _today(value: Mapping[str, Any]) -> TodayItem:
    _reject_unknown_fields(value, {"title", "detail", "status", "owner"}, "today item")
    return TodayItem(
        title=_text(value, "title"),
...
def _kanban(value: Mapping[str, Any]) -> KanbanCard:
    _reject_unknown_fields(value, {"title", "detail", "lane", "status"}, "kanban item")
    return KanbanCard(
        title=_text(value, "title"),
        detail=_text(value, "detail", max_length=MAX_LONG_TEXT_LENGTH),
...
def _project(value: Mapping[str, Any]) -> ProjectItem:
    _reject_unknown_fields(value, {"name", "summary", "status", "updated"}, "projects item")
    return ProjectItem(
...
def _approval(value: Mapping[str, Any]) -> ApprovalItem:
    _reject_unknown_fields(value, {"request", "source", "status"}, "approvals item")
    return ApprovalItem(
...
def _workflow(value: Mapping[str, Any]) -> WorkflowItem:
    _reject_unknown_fields(value, {"name", "stage", "status", "next_step"}, "workflows item")
    return WorkflowItem(
        name=_text(value, "name"),
...
def _evidence(value: Mapping[str, Any]) -> EvidenceItem:
    _reject_unknown_fields(value, {"label", "reference", "status"}, "evidence item")
    return EvidenceItem(
...
def _health(value: Mapping[str, Any]) -> HealthItem:
    _reject_unknown_fields(value, {"component", "status", "detail"}, "health item")
    return HealthItem(
...
def _reject_unknown_fields(value: Mapping[str, Any], allowed: set[str], context: str) -> None:
    ...
    if unknown:
        raise ControlBoardValidationError(f"{context} contains unknown fields: {', '.join(unknown)}")
