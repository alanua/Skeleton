...
class DurableJournal:
    ...
    def complete(self, key: str, worker_id: str, result: Mapping[str, object]) -> None:
        now = self.clock(); status = str(result.get("status", "BLOCKED")); reason = str(result.get("reason_code", "processing_failed")); state = "DONE" if status == "DONE" else "REVIEW" if status == "REVIEW" else "QUARANTINED"
        with self.store.locked() as payload:
            item = _items(payload).get(key)
            if not isinstance(item, dict) or item.get("state") != "PROCESSING" or item.get("worker_id") != worker_id: raise RuntimeErrorCode("lease_owner_mismatch")
    ...
    def health(self) -> dict[str, object]:
        ...
        blocked, degraded = counts.get("QUARANTINED", 0) > 0, counts.get("REVIEW", 0) > 0 or counts.get("RETRY", 0) > 0
...
class FamilyDocumentWorker:
    def __init__(self, *, roots: Sequence[ApprovedRoot], journal: DurableJournal, processor: Processor, lock_path: Path, worker_id: str = "family-document-worker-1") -> None:
        self.roots, self.journal, self.processor, self.lock_path, self.worker_id = tuple(roots), journal, processor, Path(lock_path), worker_id
    ...
    def run_once(self) -> dict[str, object]:
        with self.single_instance():
            discovered, recovered, settled = self.journal.discover(self.roots), self.journal.recover_expired(), self.journal.settle(self.roots); claimed = self.journal.claim(self.worker_id)
            if claimed is None: return {**self.journal.health(), "operation": "IDLE", "discovered": discovered, "recovered": recovered, "settled": settled}
            key, item = claimed
            try:
                ...
                if status == "BLOCKED": self.journal.fail(key, self.worker_id, reason, permanent=reason not in _TRANSIENT_PROCESS_REASONS)
                else: self.journal.complete(key, self.worker_id, result)
            except (SourceError, RuntimeErrorCode) as exc: self.journal.fail(key, self.worker_id, exc.reason_code, permanent=True)
            except Exception: self.journal.fail(key, self.worker_id, "processing_failed", permanent=False)
