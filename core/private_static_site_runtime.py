...
RESULT_CERTIFICATE_FENCE_RE = re.compile(
    r"(?ms)^```result-certificate-pem[ \t]*\n(?P<cert>-----BEGIN CERTIFICATE-----\n[A-Za-z0-9+/=\r\n]+-----END CERTIFICATE-----)\n```[ \t]*$"
)
PRIVATE_KEY_MARKERS = (
    "-----BEGIN PRIVATE KEY-----",
...
def deploy_private_static_site(
    ...
    *,
    ...
    try:
        ...
        result_der = _validate_result_certificate(
            result_cert_path,
            result_der_path,
            request.result_certificate_der_sha256 or "",
            runner,
        )
        ...
        if _tailscale_path_configured(previous_status, request.url_path or ""):
            return _deploy_blocked(request.artifact_id, True, True, "tailscale_path_already_configured")
        ...
        if dns_name is None:
            _rollback_tailscale_path(runner, request.url_path or "")
            _restore_published_site(current, previous_current)
        ...
        code, _ = runner(
            [
                ...
                "DER",
                "-in",
                str(result_plaintext_path),
                "-out",
                str(result_ciphertext_path),
                str(result_cert_path),
            ],
            None,
            OPENSSL_TIMEOUT,
        )
        if code != 0:
            _rollback_tailscale_path(runner, request.url_path or "")
            _restore_published_site(current, previous_current)
            return _deploy_blocked(request.artifact_id, True, True, "result_encrypt_failed")
        ...
        try:
            ...
            _rollback_tailscale_path(runner, request.url_path or "")
            _restore_published_site(current, previous_current)
            return _deploy_blocked(request.artifact_id, True, True, "metadata_write_failed")
        ...
        text = str(exc)
        reason = text if text.startswith(("zip_", "result_certificate_")) else "invalid_payload"
...
def _deploy_request(body: str) -> tuple[StaticSiteRequest | None, str | None]:
    ...
    result_certificate_hash = _body_field(body, "Result Certificate DER SHA-256")
    approval = _body_field(body, "Operator Approval")
    payload_match = PAYLOAD_FENCE_RE.search(body or "")
    result_cert_match = RESULT_CERTIFICATE_FENCE_RE.search(body or "")
    if artifact_id is None:
        return None, "missing_artifact_id"
    if ARTIFACT_ID_RE.fullmatch(artifact_id) is None:
        return None, "invalid_artifact_id"
    ...
    if result_certificate_hash is None or SHA256_RE.fullmatch(result_certificate_hash) is None:
        return None, "invalid_result_certificate_der_sha256"
    if result_cert_match is None:
        return None, "missing_result_certificate"
    result_certificate_pem = result_cert_match.group("cert")
    if any(marker in result_certificate_pem for marker in PRIVATE_KEY_MARKERS):
    ...
    return (
        StaticSiteRequest(
            ...
            result_certificate_der_sha256=result_certificate_hash,
            result_certificate_pem=result_certificate_pem,
...
def _validate_and_extract_zip(zip_path: Path, extract_root: Path) -> tuple[Path, list[str]]:
    ...
    with zipfile.ZipFile(zip_path) as archive:
        ...
        for info in infos:
            ...
            if info.is_dir():
                target_dir = extract_root / Path(*posix.parts)
...
def _tailscale_serve_status(runner: CommandRunner) -> dict[str, object] | None:
    ...
    if code != 0:
        return None
    if len(output.encode("utf-8", errors="ignore")) > MAX_TAILSCALE_STATUS_BYTES:
        return None
    try:
...
def _tailscale_path_configured(status: dict[str, object] | None, url_path: str) -> bool:
    if status is None:
        return False
...
def _rollback_tailscale_path(runner: CommandRunner, url_path: str) -> None:
    _remove_tailscale_path(runner, url_path)


def _remove_tailscale_path(runner: CommandRunner, url_path: str) -> tuple[int, str]:
    command = ["tailscale", "serve", "--bg", "--yes", "--https=443", f"--set-path={url_path}", "off"]
    code, output = runner(command, None, TAILSCALE_TIMEOUT)
    if code == 0:
        return code, output
    if "permission" in output.lower() or "sudo" in output.lower():
...
def _validate_result_certificate(
    cert_path: Path,
    der_path: Path,
...
def _result_payload_json(
    *,
    private_url: str,
    artifact_id: str,
    verification_status: str,
    asset_count: int,
    ciphertext_hash_match: bool,
    plaintext_hash_match: bool,
    cleanup_status: str,
) -> str:
    payload = {
        "artifact_id": artifact_id,
...
def _report(
    ...
    *,
    ...
    encrypted_result_cms_b64: str | None = None,
) -> str:
    heading = (
