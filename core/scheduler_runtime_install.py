...
def install_scheduler_runtime(
    ...
    *,
    ...
    previous_service = _read_optional(layout.systemd_dir / SERVICE_UNIT)
    previous_timer = _read_optional(layout.systemd_dir / TIMER_UNIT)
...
def _verify_source(source_root: Path, expected_sha: str) -> str:
    ...
    if normalized != CANONICAL_ORIGIN:
...
def _install_release(source_root: Path, release: Path) -> None:
    if release.exists():
        if (release / ".source-ready").is_file():
            return
        shutil.rmtree(release)
    temporary = release.with_name(release.name + ".part")
    shutil.rmtree(temporary, ignore_errors=True)
    temporary.mkdir(parents=True, mode=0o700)
    for name in ("core", "scripts", "schemas"):
        shutil.copytree(
            source_root / name,
            temporary / name,
            ignore=shutil.ignore_patterns("__pycache__", "*.pyc"),
        )
    shutil.copy2(source_root / "pyproject.toml", temporary / "pyproject.toml")
...
def _initialize_live_db(db_path: Path) -> str:
    ...
    required = {"schedules", "enabled", "done", "failed", "needs_operator", "pending", "running", "skipped"}
...
def _rollback(
    layout: SchedulerRuntimeLayout,
    *,
    previous_target: Path | None,
    previous_service: str | None,
    previous_timer: str | None,
) -> None:
    if previous_target is None:
        layout.current.unlink(missing_ok=True)
    else:
        _atomic_symlink(previous_target, layout.current)
    _restore_text(layout.systemd_dir / SERVICE_UNIT, previous_service)
    _restore_text(layout.systemd_dir / TIMER_UNIT, previous_timer)
...
def failure_receipt(expected_sha: str, reason_code: str) -> dict[str, object]:
    reason = reason_code if _SAFE_REASON_RE.fullmatch(reason_code) else "SCHEDULER_RUNTIME_FAILED"
