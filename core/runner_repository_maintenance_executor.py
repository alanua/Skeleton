...
import core.home_edge.esp_lab_stage1_signer_install as _signer_install_module
from core.home_edge.esp_lab_stage1_signer_install import (
    ...
    HOME_EDGE_ESP_LAB_STAGE1_SIGNER_MAX_INSTALLER_BYTES,
    ...
    HOME_EDGE_ESP_LAB_STAGE1_SIGNER_PROTECTED_PARENT_MODE,
...
_CANONICAL_VERIFY_PROTECTED_INSTALLER_PARENT = (
    _signer_install_module._verify_protected_installer_parent
)
_CANONICAL_VERIFY_REGULAR_FILE = _signer_install_module._verify_regular_file


def _verify_protected_installer_parent(*, allow_absent: bool = False) -> None:
    _signer_install_module.HOME_EDGE_ESP_LAB_STAGE1_SIGNER_PROTECTED_INSTALLER = (
        HOME_EDGE_ESP_LAB_STAGE1_SIGNER_PROTECTED_INSTALLER
    )
    _CANONICAL_VERIFY_PROTECTED_INSTALLER_PARENT(allow_absent=allow_absent)


_COMPAT_VERIFY_PROTECTED_INSTALLER_PARENT = _verify_protected_installer_parent
...
def execute_home_edge_esp_lab_stage1_signer_install(
    *,
    ...
    _signer_install_module.HOME_EDGE_ESP_LAB_STAGE1_SIGNER_INSTALLED_ARTIFACTS = (
        HOME_EDGE_ESP_LAB_STAGE1_SIGNER_INSTALLED_ARTIFACTS
    )
    _signer_install_module._verify_regular_file = _verify_regular_file
    _signer_install_module._verify_protected_installer_parent = (
        _verify_protected_installer_parent
        if _verify_protected_installer_parent is not _COMPAT_VERIFY_PROTECTED_INSTALLER_PARENT
        else _CANONICAL_VERIFY_PROTECTED_INSTALLER_PARENT
    )
    return _signer_install_module.execute_home_edge_esp_lab_stage1_signer_install(
        expected_main_sha=expected_main_sha,
        registered_clean_main_sha=registered_clean_main_sha,
        github_main_sha=github_main_sha,
        checkout_path=checkout_path,
        ...
        protected_run_command=protected_run_command,
            ...
            if duplicate:
                try:
                    ...
                    reconciled = {
                        ...
                        if key not in {"ota", "updated_at"}
                    }
                    reconciled["status"] = "BUILT"
                    reconciled["public_status"] = "artifact_built_pending_home_edge_ota"
                    reconciled["reconciliation"] = "stale_done_reconciled_to_pending_home_edge_ota"
                    history = list(reconciled.get("reconciliation_history", []))
                    history.append({"at": _utc_now(), "reason": exc.reason_code})
                    reconciled["reconciliation_history"] = history
                    _atomic_write_json(manifest_path, reconciled)
                    receipt = _blocked_receipt(exc.reason_code)
                    receipt["reconciled_manifest_status"] = "BUILT"
                    receipt["build_actions_after_reconciliation"] = 0
                    receipt["ota_actions_after_reconciliation"] = 0
                    return 0, _repository_result("BLOCKED", receipt)
            else:
