...
IDEMPOTENCY_KEY = "home-edge-01-post-migration-reconcile-20260808-v4"
...
GALLERY_VERIFY = "/home/oleksii/.local/bin/home-edge-screensaver-current-media-verify"
...
restore_all() {
  ...
  [ "$system_units_changed" = true ] && manager_reload_for system || {
    [ "$system_units_changed" = true ] && failed=true
  }
  [ "$user_units_changed" = true ] && manager_reload_for user || {
    [ "$user_units_changed" = true ] && failed=true
  }
...
if last.get("overall") != "healthy" or last.get("healthy") is not True:
    sys.exit(2)
if critical != 0 or warnings != 0:
    sys.exit(2)
PY
)" || return 1
  IFS="$(printf '\t')" read -r watchdog_critical_count watchdog_warning_count <<EOF_WATCHDOG
$parsed
EOF_WATCHDOG
  [ "$watchdog_critical_count" = "0" ] && [ "$watchdog_warning_count" = "0" ]
}
...
[ "$system_units_changed" = true ] && manager_reload_for system || {
  [ "$system_units_changed" = true ] && fail_after_mutation system_daemon_reload_failed
}
[ "$user_units_changed" = true ] && manager_reload_for user || {
  [ "$user_units_changed" = true ] && fail_after_mutation user_daemon_reload_failed
}
stale_after_count=0
while IFS= read -r -d '' scoped_path; do
