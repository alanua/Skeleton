...
FIELD_RE = re.compile(r"^(?P<field>[A-Za-z][A-Za-z0-9 _-]{0,80}): (?P<value>.*)$")
...
def parse_runtime_input(body: str) -> dict[str, str]:
    ...
    for field, expected in AUTHORITY_FIELDS.items():
        if fields.get(field) != expected:
            raise ValueError(_field_reason(field))
    return fields


def validate_trusted_main_sha(
    *,
    registered_clean_main_sha: str,
    github_main_sha: str,
) -> None:
    ...
    if registered_clean_main_sha != github_main_sha:
        raise ValueError("trusted_main_sha_mismatch")
...
def build_signed_display_off_request(
    *,
    environment: Mapping[str, str] | None = None,
) -> HomeEdgeExecRequest:
    secret = resolve_exec_hmac_secret(environment=environment)
...
def validate_signed_display_off_request(
    ...
    except HomeEdgeExecError as exc:
        raise ValueError("display_power_off_signed_request_authority_mismatch") from exc
    checks = (
...
def _field_reason(field: str) -> str:
    return {
        "Mode": "runtime_mode_mismatch",
        "Maintenance Task ID": "maintenance_task_id_mismatch",
        "Operator Approval": "operator_approval_mismatch",
        "Target Node": "target_node_mismatch",
        "Privacy Boundary": "privacy_boundary_mismatch",
    }[field]
