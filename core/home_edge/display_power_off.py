...
IDEMPOTENCY_KEY = "display-off-unified-controller-runtime-contract-20260809-v1"
...
def parse_runtime_input(body: str) -> dict[str, str]:
    ...
    if fields.get("Operator Approval") != OPERATOR_APPROVAL:
        raise ValueError("operator_approval_mismatch")
    if fields.get("Target Node") != TARGET_NODE:
        raise ValueError("target_node_mismatch")
    if fields.get("Privacy Boundary") != PRIVACY_BOUNDARY:
        raise ValueError("privacy_boundary_mismatch")
    return fields


def _metadata_lines(body: str) -> list[str]:
    return (body or "").split("```task", 1)[0].splitlines()
...
def validate_main_sha(
    *,
    ...
    for value in (registered_clean_main_sha, github_main_sha):
