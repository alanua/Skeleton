...
def run_audited_home_edge_command(
    ...
    *,
    ...
    if command == "gateway_capabilities":
        return gateway_contract(node)
    if command == "prepare_runtime_bootstrap":
        return prepared_runtime_bootstrap(node)

    artifact = run_home_edge_diagnostic(profile=node, artifact_path=artifact_path, transport=transport)
    ...
    projections: dict[str, tuple[str, Any]] = {
        ...
        "home_automation_inventory": ("home_automation", runtime_data.get("home_automation")),
...
def compact_status(artifact: dict[str, Any]) -> dict[str, str]:
    ...
    return {
        "node": str(artifact.get("node", {}).get("node_id", "home-edge-template-01")),
