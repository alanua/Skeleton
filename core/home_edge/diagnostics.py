...
def run_home_edge_diagnostic(
    *,
    ...
    active_transport = transport or OpenSSHTransport(profile=node)
    probe = REMOTE_PROBE.replace("__CONTROLLER_IP__", json.dumps(node.controller_tailscale_ip))
    attempt = active_transport.run_probe(probe, timeout_seconds=timeout_seconds)
    ...
    report = build_diagnostic_artifact(node, remote, transport_evidence=attempt.public_evidence())
...
def build_diagnostic_artifact(
    ...
    *,
    ...
    return {
        ...
        "source": "runner_remote_probe" if runtime_state == "observed" else "repository_template",
        "evidence": {
            "registration": {
                "state": "template",
        ...
        "summary": {
            ...
            "gateway": {
                ...
                "target": {"state": "template", "value": profile.target},
                "transport": transport_public,
                "contract": gateway_contract(profile),
            },
            ...
            "next_prepared_action": prepared_runtime_bootstrap(profile),
        },
    }
...
def summarize_tailscale(profile: HomeEdgeProfile, remote: dict[str, Any] | None) -> dict[str, Any]:
    expected = {"state": "template", "ip": profile.tailscale_ip}
...
def summarize_modem(remote: dict[str, Any] | None) -> dict[str, Any]:
    registered = {"state": "template", "role": "optional_external_or_attached_usb_modem"}
...
def summarize_capabilities(profile: HomeEdgeProfile, remote: dict[str, Any] | None) -> dict[str, Any]:
    registered = {"state": "template", "capabilities": list(profile.capabilities)}
    if not isinstance(remote, dict):
        return {
            "status": "unverified",
            "evidence": "unverified",
    ...
    return {
        ...
        "evidence": "observed",
        "registered": registered,
        "observed": {"state": "observed", "value": observed if isinstance(observed, dict) else {}},
    }
...
def build_operator_report(artifact: dict[str, Any]) -> str:
    ...
    return "\n".join(
        (
            ...
            f"node={artifact.get('node', {}).get('node_id', 'home-edge-template-01')}",
...
report = {
    ...
    "network": {"default_route": default_route(), "route_to_controller": run(["ip", "route", "get", CONTROLLER_IP], timeout=5)["ok"]},
