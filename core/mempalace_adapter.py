...
class MemPalaceAdapter:
    ...
    def canonical_exact_for_target(
        self, *, namespace: str, project_id: str, normalized_target: str
    ) -> dict[str, object] | None:
        self._authorize_scope(namespace=namespace, project_id=project_id)
        for document in self._projection.documents:
            if document.deleted:
                continue
            if normalized_target not in {document.item_id, document.canonical_ref}:
                continue
            return {
                ...
                "provenance_refs": [
                    {
                        "ref": attribution["source_ref"],
                        "kind": "exact_source",
                        "evidence_hash": attribution["evidence_hash"],
                    }
                    for attribution in document.source_attribution
                ],
            }
        return None
