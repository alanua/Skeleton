...
RUNTIME_ENFORCEMENTS = frozenset({"runtime_gate", "preflight_gate", "route_validation"})
...
PRIVATE_REPORT_PATTERNS = (
    ...
    re.compile(
        r"\b[A-Za-z0-9_]*(?:token|secret|password|api[_-]?key)[A-Za-z0-9_]*\s*[:=]\s*\S+",
        re.IGNORECASE,
    ),
...
def validate_registry(
    ...
    *,
    ...
    errors.extend(_evidence_errors(root_path, rules))
    errors.extend(_approval_scope_errors(rules))

    gaps = _effective_enforcement_gaps(rules)
...
def build_report(registry: Mapping[str, Any] | None = None, *, root: Path | str = ROOT) -> str:
    ...
    lines.extend(_id_lines(result.effective_enforcement_gaps))
...
def _evidence_errors(root: Path, rules: Iterable[Mapping[str, Any]]) -> list[str]:
    errors: list[str] = []
    for rule in rules:
        ...
        if isinstance(source_file, str) and isinstance(source_locator, str):
            errors.extend(_locator_errors(root, source_file, source_locator, rule_id, "source_locator"))
        for field in ("test_evidence", "audit_evidence"):
            ...
            for value in values:
                ...
                if ":" in value:
                    ...
                    errors.extend(_locator_errors(root, path_text, locator, rule_id, field))
                elif not (root / value).exists():
                    errors.append(f"{rule_id} {field} references missing path: {value}")
        gate_entrypoint = str(rule.get("gate_entrypoint", "")).strip()
        if rule.get("enforcement") in RUNTIME_ENFORCEMENTS:
            if not gate_entrypoint:
                errors.append(f"{rule_id} verifiable rule lacks gate_entrypoint")
            elif gate_entrypoint.endswith((".yaml", ".yml", ".json", ".md")):
                if not (root / gate_entrypoint).exists():
                    errors.append(f"{rule_id} gate_entrypoint references missing path: {gate_entrypoint}")
            elif ":" in gate_entrypoint:
                path_text, locator = gate_entrypoint.split(":", 1)
                errors.extend(_locator_errors(root, path_text, locator, rule_id, "gate_entrypoint"))
            else:
                errors.extend(_python_callable_errors(root, gate_entrypoint, rule_id))
            reason_tokens = rule.get("reason_tokens")
            if not isinstance(reason_tokens, list) or not reason_tokens:
                errors.append(f"{rule_id} verifiable rule lacks stable reason token")
    return errors
...
def _effective_enforcement_gaps(rules: Iterable[Mapping[str, Any]]) -> tuple[str, ...]:
    gaps = []
    for rule in rules:
        if rule.get("lifecycle") != "active":
            continue
        if rule.get("enforcement") not in EFFECTIVE_ENFORCEMENTS:
