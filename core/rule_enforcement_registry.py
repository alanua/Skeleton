...
def validate_registry(
    ...
    *,
    ...
    errors: list[str] = []
    schema_input_errors = _schema_input_errors(schema_data)
    errors.extend(schema_input_errors)
    if not schema_input_errors:
    ...
    for rule in rules:
        ...
        if rule.get("enforcement") in EFFECTIVE_ENFORCEMENTS:
            ...
            if not isinstance(reason_tokens, list) or not reason_tokens:
                ...
                missing_tokens = _missing_reason_token_evidence(root, rule, reason_tokens)
                rule_errors.extend(
                    f"{rule_id} reason token lacks concrete evidence: {token}" for token in missing_tokens
                )
...
def _schema_input_errors(schema: Mapping[str, Any]) -> list[str]:
    required = {"$schema", "$id", "type", "additionalProperties", "required", "properties", "$defs"}
    if not schema:
        return ["schema mapping is empty or structurally invalid"]
    missing = sorted(required - set(schema))
    errors = [f"schema mapping is structurally invalid; missing {field}" for field in missing]
    if schema.get("type") != "object":
        errors.append("schema mapping is structurally invalid; type must be object")
    if schema.get("additionalProperties") is not False:
        errors.append("schema mapping is structurally invalid; additionalProperties must be false")
    for field in ("required",):
        if field in schema and not isinstance(schema.get(field), list):
            errors.append(f"schema mapping is structurally invalid; {field} must be an array")
    for field in ("properties", "$defs"):
        if field in schema and not isinstance(schema.get(field), Mapping):
            errors.append(f"schema mapping is structurally invalid; {field} must be a mapping")
    return errors


def _missing_reason_token_evidence(
    root: Path, rule: Mapping[str, Any], reason_tokens: Iterable[object]
) -> tuple[str, ...]:
    evidence_text = "\n".join(_reason_token_evidence_texts(root, rule))
    normalized = _evidence_tokenize(evidence_text)
    missing = [
        str(token)
        for token in reason_tokens
        if isinstance(token, str) and token not in normalized
    ]
    return tuple(sorted(missing))


def _reason_token_evidence_texts(root: Path, rule: Mapping[str, Any]) -> list[str]:
    texts: list[str] = []
    source_file = rule.get("source_file")
    source_locator = rule.get("source_locator")
    if isinstance(source_file, str) and isinstance(source_locator, str):
        texts.extend(_locator_text(root, source_file, source_locator))
    gate_entrypoint = rule.get("gate_entrypoint")
    if isinstance(gate_entrypoint, str) and gate_entrypoint.strip():
        texts.extend(_entrypoint_text(root, gate_entrypoint))
    for field in ("test_evidence", "audit_evidence"):
        values = rule.get(field)
        if not isinstance(values, list):
            continue
        for value in values:
            ...
            if ":" in value:
                path_text, locator = value.split(":", 1)
                texts.extend(_locator_text(root, path_text, locator))
            else:
                path = root / value
                if path.exists() and path.is_file() and path.suffix in {".md", ".yaml", ".yml", ".json", ".py"}:
                    texts.append(path.read_text(encoding="utf-8"))
    return texts


def _entrypoint_text(root: Path, entrypoint: str) -> list[str]:
    if ":" in entrypoint:
        path_text, locator = entrypoint.split(":", 1)
        return _locator_text(root, path_text, locator)
    ...
    return _python_member_text(module_path, ".".join(member_parts)) if module_path.exists() else []


def _locator_text(root: Path, source_file: str, locator: str) -> list[str]:
    path = root / source_file
    if not path.exists() or not path.is_file():
        return []
    if path.suffix == ".py":
        return _python_member_text(path, locator)
    if path.suffix in CONFIG_SUFFIXES:
        return _config_locator_text(path, locator)
    text = path.read_text(encoding="utf-8")
    return [line for line in text.splitlines() if locator in line] or [text]


def _python_member_text(path: Path, locator: str) -> list[str]:
    source = path.read_text(encoding="utf-8")
    tree = ast.parse(source)
    parts = locator.split(".")
    for node in tree.body:
        if len(parts) == 1 and isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)) and node.name == parts[0]:
            return [source]
        if len(parts) == 2 and isinstance(node, ast.ClassDef) and node.name == parts[0]:
...
def _config_locator_text(path: Path, locator: str) -> list[str]:
...
def _evidence_tokenize(text: str) -> str:
...
def _supersession_reference_errors(rules: Iterable[Mapping[str, Any]]) -> list[str]:
    ...
    by_id = {str(rule.get("rule_id")): rule for rule in rule_list}
    ids = set(by_id)
    errors: list[str] = []
    for rule in rule_list:
        ...
        if isinstance(supersedes, list):
            for value in supersedes:
                if value not in ids:
                    errors.append(f"{rule_id} supersedes unknown rule_id: {value}")
                    continue
                target = by_id[str(value)]
                if rule_id not in _string_list(target.get("superseded_by")):
                    errors.append(f"{rule_id} supersedes lacks reciprocal superseded_by on {value}")
                if target.get("lifecycle") != "superseded":
                    errors.append(f"{rule_id} supersedes non-superseded rule: {value}")
                if rule.get("lifecycle") == "active" and target.get("lifecycle") == "active":
                    errors.append(f"{rule_id} active-to-active supersession is invalid: {value}")
        superseded_by = rule.get("superseded_by")
        if isinstance(superseded_by, list):
            if superseded_by and rule.get("lifecycle") != "superseded":
                errors.append(f"{rule_id} superseded_by requires lifecycle superseded")
            for value in superseded_by:
                if value not in ids:
                    errors.append(f"{rule_id} superseded_by unknown rule_id: {value}")
                    continue
                replacement = by_id[str(value)]
                if replacement.get("lifecycle") == "superseded":
                    errors.append(f"{rule_id} superseded_by points to superseded replacement: {value}")
                if rule_id not in _string_list(replacement.get("supersedes")):
                    errors.append(f"{rule_id} superseded_by lacks reciprocal supersedes on {value}")
                if rule.get("lifecycle") == "active" and replacement.get("lifecycle") == "active":
                    errors.append(f"{rule_id} active-to-active supersession is invalid: {value}")
    return errors
