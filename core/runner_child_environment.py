...
# This wrapper is now intentionally Codex-only. External-model execution belongs
# to ExecutionBinding/RouteLease and must never be an implicit child fallback.
...
def _install_fallback_wrapper(
    ...
    """Bind child codegen only to the canonical recovered Codex runtime.

    The historical function name is preserved for compatibility. It no longer
    installs or authorizes an OpenHands/model-provider fallback; that authority
    belongs exclusively to the Execution Fabric.
    """
