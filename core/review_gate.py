...
AUTHORIZED_WORKFLOW_CONTINUATION_STEP: Final = "authorized_workflow_continuation"
...
def dispatch_internal_review_control(
    ...
    *,
    ...
    if next_step == "bounded_repair_replacement_pr":
    ...
    if next_step == AUTHORIZED_WORKFLOW_CONTINUATION_STEP:
...
def continue_from_internal_review_verdict(
    ...
    *,
    ...
    merge_policy_verdict: str = AUTO_MERGE_ALLOWED,
) -> ReviewContinuation:
    normalized = str(verdict or "").strip().upper()
    normalized_policy = str(merge_policy_verdict or "").strip().upper()
    ...
    if normalized == APPROVE and normalized_policy == NEVER_AUTO:
        normalized = DO_NOT_MERGE
        policy_reason = "policy_never_auto"
    if normalized == APPROVE and normalized_policy != AUTO_MERGE_ALLOWED:
        normalized = NEEDS_OPERATOR
        policy_reason = "operator_authority_required"

    if normalized == APPROVE:
        next_step = AUTHORIZED_WORKFLOW_CONTINUATION_STEP
    elif normalized == REQUEST_CHANGES:
        next_step = "bounded_repair_replacement_pr"
    ...
    payload.update(
        {
            ...
            "merge_policy_authority": _merge_policy_authority(
                verdict=normalized,
                merge_policy_verdict=normalized_policy,
                head_sha=expected_head_sha,
                protected=protected,
            ),
        }
    ...
    payload.update(
        {
            ...
            "replacement_head_sha": _normalize_head(replacement_head_sha),
            "reviewed_pr_number": reviewed_pr_number,
            "reviewed_head_sha": (
                None if reviewed_head_sha is None else _normalize_head(reviewed_head_sha)
            ),
        }
    )
    return enqueue_internal_review_control(store, payload=payload, now=now)
...
def enqueue_internal_review_control(
    ...
    *,
    ...
    scheduled_for = int(payload.get("scheduled_for") or 0)
    ...
    occurrence_id = stable_occurrence_id(
        stored.spec.schedule_id, stored.version, scheduled_for
    )
    proposal = build_execution_proposal(
        stored, occurrence_id=occurrence_id, scheduled_for=scheduled_for
    )
...
def _dispatch_repair_done(
    ...
    *,
    ...
    if control.get("replacement_pr_number") != control.get("pr_number"):
        return _blocked_receipt("REPAIR_REPLACEMENT_PR_UNVERIFIED")
    if control.get("replacement_head_sha") != control.get("head_sha"):
        return _blocked_receipt("REPAIR_REPLACEMENT_HEAD_UNVERIFIED")
    ...
    if existing is None:
        _, created = store.record_operational_event_once(
            ...
            payload={
                ...
                "reviewed_pr_number": control.get("reviewed_pr_number"),
                "reviewed_head_sha": control.get("reviewed_head_sha"),
                "replacement_pr_number": control.get("replacement_pr_number"),
                "replacement_head_sha": control.get("replacement_head_sha"),
                "public_safe": True,
                "external_side_effects_executed": False,
            },
            now=now,
    ...
    return {
        ...
        "accepted": True,
        "decision": "ACCEPT",
        "reason": "repair_done_verified" if created else "repair_done_reused",
        ...
        "replacement_pr_number": control.get("replacement_pr_number"),
        "replacement_head_sha": control.get("replacement_head_sha"),
        "materialized_continuation": materialized,
        "public_safe": True,
        "external_side_effects_executed": False,
    }
...
def _dispatch_authorized_continuation(
    ...
    *,
    ...
    if control.get("review_verdict") != APPROVE:
        return _blocked_receipt("AUTHORIZED_CONTINUATION_REQUIRES_APPROVE")
    authority = _mapping(control.get("merge_policy_authority"))
    if authority.get("verdict") != AUTO_MERGE_ALLOWED:
        return _blocked_receipt("AUTHORIZED_CONTINUATION_POLICY_MISMATCH")
    if authority.get("operator_approval") is not False:
        return _blocked_receipt("AUTHORIZED_CONTINUATION_OPERATOR_AUTHORITY_PRESENT")
    if authority.get("protected_authority") is not False:
        return _blocked_receipt("AUTHORIZED_CONTINUATION_PROTECTED_AUTHORITY_PRESENT")
    if control.get("protected") is True:
        return _blocked_receipt("AUTHORIZED_CONTINUATION_PROTECTED_SCOPE")
    if adapter is None:
        return _blocked_receipt("ACTION_AUTHORITY_UNAVAILABLE")

    packet = _control_packet(control, action=AUTHORIZED_WORKFLOW_CONTINUATION_STEP)
    packet.update(
        {
            "merge_policy_authority": dict(authority),
            "operator_notification_allowed": False,
            "merge_executed": False,
        }
    )
    ledger_key = control_action_ledger_key(
        action=AUTHORIZED_WORKFLOW_CONTINUATION_STEP,
        repository=str(control["repository"]),
        pr_number=int(control["pr_number"]),
        head_sha=str(control["head_sha"]),
    ...
    if existing is None:
        ...
        event, created = store.record_operational_event_once(
            ...
            payload={
                "schema": "skeleton.internal_review_gate.authorized_continuation.v1",
                "packet": packet,
                "adapter_receipt": dict(adapter_receipt),
                "public_safe": True,
                "external_side_effects_executed": False,
        ...
        event = existing
        created = False
        adapter_receipt = {"status": "DONE", "reused_existing": True}

    receipt = _terminal_control_receipt(
        control, next_step=AUTHORIZED_WORKFLOW_CONTINUATION_STEP
    )
    receipt.update(
        {
            "status": "DONE",
            "decision": "ACCEPT",
            "internal_review_verdict": APPROVE,
            "merge_policy_authority": dict(authority),
            "authorized_continuation": packet,
            "authorized_event": event,
            "authorized_continuation_recorded": created,
            "adapter_receipt": dict(adapter_receipt),
            "operator_notification_allowed": False,
        }
    )
...
def _repair_task(
    ...
    return {
        ...
        "uses_existing_pr": False,
        "repair_model": "replacement_pr_from_current_main",
        "supersedes_pr_number": request.pr_number,
        "supersedes_head_sha": _head_sha(request.pr),
        "public_safe": True,
    }
...
def _repair_task_from_control(control: Mapping[str, Any]) -> dict[str, object]:
    ...
    return {
        ...
        "supersedes_pr_number": int(control["pr_number"]),
        "supersedes_head_sha": _normalize_head(str(control["head_sha"])),
        "repair_model": "replacement_pr_from_current_main",
        "source_issue": control["source_issue"],
        "allowed_files": tuple(str(path) for path in control["allowed_files"]),
...
def _merge_policy_authority(
    *,
    verdict: str,
    merge_policy_verdict: str,
    head_sha: str,
    protected: bool,
    ...
    if verdict != APPROVE:
        return None
    return {
        "schema": "skeleton.internal_review_gate.merge_policy_authority.v1",
        "source": "DelegatedMergePolicyChecker",
        "verdict": merge_policy_verdict,
        "head_sha": _normalize_head(head_sha),
        "scope": "non_protected_internal",
        "operator_approval": False,
        "trusted_approval_reference": None,
