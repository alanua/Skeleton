...
def dispatch_internal_review_control(
    ...
    *,
    ...
    if next_step == "internal_review":
        return _dispatch_internal_review(control, store=store, now=now, state_reader=state_reader)
    if next_step == "bounded_repair_existing_pr_branch":
        return _dispatch_repair(control, store=store, now=now)
    if next_step in {
        "authorized_merge_continuation",
        "internal_repair_supersede_dependency",
...
def _dispatch_internal_review(
    ...
    *,
    ...
    protected = decision.verdict == NEEDS_OPERATOR or decision.operator_packet is not None
    continuation = continue_from_internal_review_verdict(
        ...
        now=now,
        protected=protected and decision.verdict == APPROVE,
    )
    notification_claimed = False
    if continuation.payload.get("operator_notification_allowed") is True:
        notification_claimed = claim_operator_notification(
            store,
            status=NEEDS_OPERATOR,
            now=now,
            repository=request.repository,
            ...
            reason=str(continuation.payload.get("policy_reason") or decision.reason),
            payload=durable_needs_operator_result(
                repository=request.repository,
                pr_number=request.pr_number,
                head_sha=str(control["head_sha"]),
                permitted_merge_method="squash",
                policy_reason=str(continuation.payload.get("policy_reason") or decision.reason),
                next_step=str(continuation.payload.get("next_step") or "operator_review_required"),
            ),
        )
    ...
    receipt.update(
        {
            ...
            "operator_notification_claimed": notification_claimed,
            "public_safe": True,
            "external_side_effects_executed": False,
        }
    )
...
def _dispatch_repair(
    ...
    *,
    ...
    rereview["repair_parent_reason"] = str(control.get("policy_reason") or "repair_completed")
    ...
    return {
        ...
        "accepted": True,
        "decision": "ACCEPT",
        "reason": "repair_control_completed",
