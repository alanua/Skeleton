...
artifact under the configured private host-maintenance root. The link is the
owner handoff intended for delivery over Viber or another private channel. The
private HTTPS endpoint serves one owner enrollment payload matching
`schemas/remote_windows_owner_enrollment.schema.json`; the Windows bootstrap
uses that payload to install the controller public key without asking the owner
to paste it. The public JSON report contains only the enrollment id, private
artifact reference, SHA-256 hashes, delivery-channel token, token-lifecycle
marker, status tokens, and `target_enrolled: false`. The token has no automatic
expiry, TTL, or `EXPIRED` state; rotate or revoke it manually after successful
enrollment or cancellation. It must not
include the HTTPS link, the enrollment code, target host identifiers, passwords,
