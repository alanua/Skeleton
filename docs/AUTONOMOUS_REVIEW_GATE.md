...
- `APPROVE` enters the gated authorized continuation path only for unprotected
  current-head PRs. It does not raw-merge from the review dispatcher.
- `REQUEST_CHANGES` creates or reuses one public-safe `agent:task` repair issue
  tied to repository, PR, head SHA, allowed files, validations, policy reason,
  and deterministic idempotency key. It waits for verified repair publication
  before scheduling exactly one re-review.
