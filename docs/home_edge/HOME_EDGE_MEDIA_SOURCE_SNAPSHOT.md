...
Direct trusted contexts may still sign the Home Edge executor request with an explicit `SKELETON_HOME_EDGE_EXEC_HMAC_SECRET` value. The unprivileged Runner path does not read the controller HMAC config. If the explicit trusted HMAC value is absent, the Runner invokes only:

`/usr/bin/sudo --non-interactive -- /usr/local/sbin/home_edge_media_source_snapshot_signer`

with a fixed minimal environment. The signer accepts no argv and reads a minimal JSON stdin object containing only the fixed task identity plus public-safe expected main SHA metadata. It returns only a signed request envelope for `home_edge_01_media_source_snapshot_v1`; it never returns the HMAC and does not execute Home Edge transport, read source output, write artifacts, copy files, or change ownership.

Inside the privileged signer only, the fixed private config is parsed as text, never sourced or executed. The resolver reads only `/etc/skeleton/home-edge-executor-controller.env` and only the single allowlisted variable `SKELETON_HOME_EDGE_EXEC_HMAC_SECRET`. It never reads or parses `/etc/skeleton/home-edge-01.env`; that file is metadata-only corroboration for the private controller ownership boundary. The fixed `/etc/skeleton` directory plus `/etc/skeleton/home-edge-01.env` and `/etc/skeleton/home-edge-executor-controller.env` are all checked with `lstat`; symlinks are rejected, `/etc/skeleton` must be a directory, both env paths must be regular files, none may be group/world writable, and each env file must be no larger than 64 KiB. The controller env is accepted when its owner is root or the current privileged signer uid, or when those three fixed paths share one identical owner and group boundary under the same strict checks. The parser accepts only simple `KEY=VALUE` or optional `export KEY=VALUE` entries for the allowlisted variable, ignoring comments, blank lines, and unrelated assignments. Duplicate target entries, NUL bytes, malformed quoted values, shell substitution or variable references, backticks, and multiline continuations fail closed before any executor request is made.

Authentication setup failures are reported only as stable public-safe classes such as `executor_auth_config_missing`, `executor_auth_config_unsafe`, `executor_auth_config_invalid`, or bounded `controller_signer_*` reasons. The credential value, config path, variable name, signed envelope, and HMAC-derived signature are never included in public maintenance reports.

The controller signer is installed by `scripts/install_home_edge_media_source_snapshot_controller_signer.sh`, separate from the Home Edge node executor installer. Its sudoers rule must bind the actual Runner service identity to exactly `/usr/local/sbin/home_edge_media_source_snapshot_signer`, with no wildcard, `SETENV`, shell, Python, cat, or arbitrary argv permission. Runtime activation and discovery of the real Runner service identity remain protected maintenance work after reviewed merge.
...
If the existing artifact is missing, the first capture obtains one controller-signed fixed request and validates every non-volatile field before transport: task/node/lane/run user/script/interpreter/timeout/output cap/public flag, plus empty argv, cwd, stdin, environment, and approval fields. The unprivileged Runner then executes the existing `execute_home_edge_request` transport itself using the validated request. If transport fails ambiguously before the private artifact is published, the operation fails closed without retrying or writing an artifact; another capture requires a deliberate v2 or newly approved operation rather than unbounded retries.
...
The Runner independently decodes and revalidates the source from the private executor response, recomputes SHA-256 and byte count, reruns the credential assignment scanner, and atomically creates the latest local snapshot under Runner ownership only after the local hash and size match the remote public metadata. No privileged source artifact is created, copied, handed off, chmodded, or chowned.
