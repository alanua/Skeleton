...
The normal unprivileged Runner never reads, resolves, or signs with `SKELETON_HOME_EDGE_EXEC_HMAC_SECRET`. It constructs one unsigned fixed executor request, including operator approval `EXPLICIT_MINIMAL_HOME_EDGE_SNAPSHOT_ACCESS_REPAIR_2026_08_09`, validates that all authority fields are code-owned, and invokes exactly:

`/usr/bin/sudo --non-interactive -- /usr/local/sbin/home_edge_media_source_snapshot_signer`

The signer accepts no argv. Its bounded stdin is only the unsigned request schema. Missing approval, near-match approval, altered node, lane, user, timeout, output cap, public flag, cwd, environment, stdin, argv, script, interpreter, or idempotency prefix is rejected before any credential file is opened. The signer returns only a signed request envelope; it never executes transport and never reads or copies the Home Edge source or Runner private artifact.

Only the privileged signer reads `/etc/skeleton/home-edge-executor-controller.env` and only the single allowlisted variable `SKELETON_HOME_EDGE_EXEC_HMAC_SECRET`. That fixed private config is parsed as text, never sourced or executed. The signer never reads or parses `/etc/skeleton/home-edge-01.env`; that file is metadata-only corroboration for the private controller ownership boundary. The fixed `/etc/skeleton` directory plus `/etc/skeleton/home-edge-01.env` and `/etc/skeleton/home-edge-executor-controller.env` are all checked with `lstat`; symlinks are rejected, `/etc/skeleton` must be a directory, both env paths must be regular files, none may be group/world writable, and each env file must be no larger than 64 KiB. The controller env is accepted when its owner is root or the current signer uid, or when those three fixed paths share one identical owner and group boundary under the same strict checks. The parser accepts only simple `KEY=VALUE` or optional `export KEY=VALUE` entries for the allowlisted variable, ignoring comments, blank lines, and unrelated assignments. Duplicate target entries, NUL bytes, malformed quoted values, shell substitution or variable references, backticks, and multiline continuations fail closed before signing.

The Runner compares the returned unsigned request and every unsigned field inside the signed request against its original exact unsigned request before transport. Any extra or altered field fails closed before `execute_home_edge_request`. Authentication and signer setup failures are reported only as stable public-safe classes. The credential value, config path, variable name, and signed envelope are never included in public receipts.
