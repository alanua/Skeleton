...
managers before the rollback is considered applied. If a forward or rollback
daemon reload fails after unit-file mutation, the task fails closed; rollback is
not reported as applied unless restored unit files and their manager reloads are
verified.
...
Watchdog health requires real `last.summary.critical` and
`last.summary.warnings` integer counts from the runtime JSON; missing, malformed,
boolean, negative, or nonzero counts fail closed and are not defaulted.
