...
recovery path before and after the modem action, fail closed when the reserved test
profile already exists, create the `internet` APN only as a non-default profile with
autoconnect disabled, activate that profile only for a bounded connection/signal probe,
and roll back only the modem session/profile created by the current run. Public reports
are aggregate status tokens only.
