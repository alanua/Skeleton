...
the installed `home_edge_exec --server` executable for a single signed JSON
request. The repository does not define an always-running executor daemon or a
...
does not enable a restartable service or public listener. Runtime deployment
