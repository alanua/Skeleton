...
Internal review `APPROVE` can continue automatically only when its materialized
payload includes current-head, non-protected `AUTO_MERGE_ALLOWED` authority. The
scheduler records that continuation idempotently without manufacturing operator
approval, approval references, or protected authority.
