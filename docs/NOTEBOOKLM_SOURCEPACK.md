...
- `github_task_queue`: Polls GitHub issues labeled runner:ready, executes bounded Codex tasks in per-issue worktrees or explicit allowlisted host maintenance tasks on Hetzner Runner, re-checks ChatGPT review markers before allowlisted GitHub action requests, creates draft PRs for code tasks, and reports results back to GitHub. Module: `scripts/runner_poll_github_tasks.py`. Status: tested.
