...
an artifact path is explicitly supplied, and non-template runtime identity is rejected if the
artifact path resolves to the repository root or any descendant. The public checkout may only
contain synthetic templates and aggregate statuses.
