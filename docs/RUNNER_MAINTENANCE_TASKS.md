...
4. Copy only `skeleton-telegram-callback-poll.service` and
   `skeleton-telegram-callback-poll.timer` into `/etc/systemd/system`, then set
   root ownership and safe read-only unit-file permissions.
5. Reload systemd, enable and start the callback timer, and run the callback
   service once.
6. Verify that the callback timer is active and the callback service one-shot
   result is successful before reporting `DONE`.

All privileged host commands use non-interactive `sudo -n`. A missing sudo
authorization blocks the task rather than waiting for operator input.
...
status lines only. A failed maintenance step, timer activation verification, or
callback one-shot verification is `BLOCKED`, and a report that contains
`BLOCKED` or `success_criteria=not_met` must not receive the `runner:done`
label.
