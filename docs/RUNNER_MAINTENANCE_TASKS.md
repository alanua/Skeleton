...
include checkout paths, absolute host paths, or raw command output. Reports keep
the target project and target repository fields; the registered checkout path is
used only internally for validation and bounded Git commands.
...
short, human-readable, and must not include raw command output, checkout paths,
or absolute host paths. Reports keep the target project and target repository
fields; the registered checkout path is used only internally for validation and
bounded Git commands. It may include only safe synthesized status lines such as
current `main` SHA, checkout sync state, open PR/issue counts, and bounded
reminder notes.
