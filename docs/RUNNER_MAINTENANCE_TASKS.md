...
3. Back up only the bounded Graphify-managed skill paths under the local Codex
   and Hermes skill directories, using a private `0700` backup root and no
   symlink traversal.
...
5. Run a temporary synthetic AST smoke using the supported build form
   ...
   directory.
6. Run that smoke with a scrubbed environment: no model credentials, no network
   enablement, no hooks, no services, no ports, and no private indexing.
7. Roll back the bounded Codex and Hermes skill paths from the local backup if
   either skill install or the synthetic smoke fails after the backup is taken.
