...
3. Back up only the bounded Graphify-managed Codex and Hermes skill paths and
   Graphify-owned version marker files, using a private `0700` recovery root and
...
8. Roll back the bounded Codex and Hermes skill paths and Graphify-owned version
   markers from the private recovery snapshot if either skill install or the
   synthetic smoke fails after the backup is taken.
