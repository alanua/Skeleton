...
PROTECTED_BOOTSTRAP="/usr/local/libexec/skeleton/runner-controller/bootstrap/install_runner_controller_privileged_gateway.sh"
...
run_git() {
  local tmp rc size
  tmp="$(mktemp)" || block "git-output-tempfile-unavailable"
  if [[ ${EUID:-$(id -u)} -eq 0 ]]; then
    env -i HOME=/nonexistent LANG=C LC_ALL=C PATH=/usr/bin:/bin GIT_CONFIG_GLOBAL=/dev/null GIT_CONFIG_NOSYSTEM=1 GIT_TERMINAL_PROMPT=0 \
      "$RUNUSER_BIN" -u "$RUNNER_USER" -- "$GIT_BIN" -C "$CANONICAL_REPO_ROOT" "$@" >"$tmp" 2>&1
  else
    env -i HOME=/nonexistent LANG=C LC_ALL=C PATH=/usr/bin:/bin GIT_CONFIG_GLOBAL=/dev/null GIT_CONFIG_NOSYSTEM=1 GIT_TERMINAL_PROMPT=0 \
      "$GIT_BIN" -C "$CANONICAL_REPO_ROOT" "$@" >"$tmp" 2>&1
  fi
  rc=$?
  size="$(wc -c <"$tmp")" || {
    rm -f -- "$tmp"
    block "git-output-size-unavailable"
  }
...
run_remote_git() {
  ...
  if [[ ${EUID:-$(id -u)} -eq 0 ]]; then
    env -i HOME=/nonexistent LANG=C LC_ALL=C PATH=/usr/bin:/bin GIT_CONFIG_GLOBAL=/dev/null GIT_CONFIG_NOSYSTEM=1 GIT_TERMINAL_PROMPT=0 \
      "$RUNUSER_BIN" -u "$RUNNER_USER" -- "$GIT_BIN" -C / "$@" >"$tmp" 2>&1
  else
    env -i HOME=/nonexistent LANG=C LC_ALL=C PATH=/usr/bin:/bin GIT_CONFIG_GLOBAL=/dev/null GIT_CONFIG_NOSYSTEM=1 GIT_TERMINAL_PROMPT=0 \
      "$GIT_BIN" -C / "$@" >"$tmp" 2>&1
  fi
...
if [[ -z "$DESTDIR" && "$SSHD_BIN" != "/usr/sbin/sshd" ]]; then
  block "custom-sshd-not-allowed-live"
fi
...
if [[ -n "$DESTDIR" && -n "${SKELETON_GATEWAY_SYNTHETIC_REMOTE_MAIN:-}" ]]; then
  fresh_remote_output="${SKELETON_GATEWAY_SYNTHETIC_REMOTE_MAIN}	refs/heads/main"
else
  fresh_remote_output="$(run_remote_git ls-remote --exit-code "$CANONICAL_ORIGIN_HTTPS" refs/heads/main)" || block "fresh-remote-main-unavailable"
fi
fresh_remote_main="$(printf '%s\n' "$fresh_remote_output" | awk 'END { if (NR == 1 && $2 == "refs/heads/main" && $1 ~ /^[0-9a-f]{40}$/) print $1; else exit 1 }')" || block "fresh-remote-main-malformed"
...
verify_running_installer() {
  ...
  script_relative="scripts/install_runner_controller_privileged_gateway.sh"
  if [[ -z "$DESTDIR" ]]; then
    [[ "$script_canonical" == "$PROTECTED_BOOTSTRAP" ]] || block "protected-bootstrap-required"
    [[ ! -L "$script_canonical" && -f "$script_canonical" ]] || block "protected-bootstrap-unsafe"
    [[ "$(stat -c '%u:%g' -- "$script_canonical")" == "0:0" ]] || block "protected-bootstrap-owner-mismatch"
    protected_mode="$(stat -c '%a' -- "$script_canonical")"
    (( (8#$protected_mode & 8#022) == 0 )) || block "protected-bootstrap-writable"
  else
...
validate_sshd_fragment() {
  local candidate="$1"
  [[ "$SSHD_BIN" == "/usr/sbin/sshd" || -n "$DESTDIR" ]] || block "custom-sshd-not-allowed-live"
...
install_ssh_account_live() {
  if ! getent passwd "$SSH_USER" >/dev/null; then
    useradd --system --home-dir /nonexistent --shell /bin/sh --no-create-home "$SSH_USER"
  fi
  passwd -l "$SSH_USER" >/dev/null 2>&1 || true
  usermod --home /nonexistent --shell /bin/sh "$SSH_USER"
}

validate_public_key() {
  SSH_PUBLIC_KEY_CANDIDATE="$1" /usr/bin/python3 - <<'PY'
import base64
import os
import re
import sys

key = os.environ.get("SSH_PUBLIC_KEY_CANDIDATE", "")
if not key or "\n" in key or "\r" in key or key != key.strip() or len(key.encode()) > 512:
    sys.exit(2)
match = re.fullmatch(r"(ssh-ed25519|ecdsa-sha2-nistp256) ([A-Za-z0-9+/]+={0,2})(?: [A-Za-z0-9_.@:+/=-]{1,80})?", key)
if match is None:
    sys.exit(2)
try:
    decoded = base64.b64decode(match.group(2).encode("ascii"), validate=True)
except Exception:
    sys.exit(2)
prefix = len(match.group(1)).to_bytes(4, "big") + match.group(1).encode("ascii")
if len(decoded) < 32 or not decoded.startswith(prefix):
    sys.exit(2)
PY
}

install_sudoers_candidate() {
  local mode="$1" absolute_dest="$2" candidate
  install -d -m 0755 "$(dirname -- "$(target "$absolute_dest")")"
  candidate="$(mktemp "$(dirname -- "$(target "$absolute_dest")")/.sudoers.XXXXXX")" || block "sudoers-candidate-unavailable"
  cat > "$candidate"
  chmod "$mode" "$candidate"
  "$VISUDO_BIN" -cf "$candidate" >/dev/null || {
    rm -f -- "$candidate"
    block "sudoers-validation-failed"
  }
  mv -f -- "$candidate" "$(target "$absolute_dest")"
}
...
install_sudoers_candidate 0440 "$SUDOERS_PATH" <<EOF
...
validate_public_key "$SSH_PUBLIC_KEY" || block "ssh-public-key-invalid"

if [[ -z "$DESTDIR" ]]; then
  install_ssh_account_live
else
  write_file 0444 "/etc/passwd.d/skeleton-runner-gateway.plan" <<EOF
$SSH_USER:x:synthetic:synthetic:Skeleton Runner Gateway:/nonexistent:/bin/sh
EOF
fi

install -d -m 0700 "$(dirname -- "$(target "$SSH_AUTHORIZED_KEYS")")"
write_file 0600 "$SSH_AUTHORIZED_KEYS" <<EOF
command="$GATEWAY_COMMAND",restrict,no-pty,no-port-forwarding,no-X11-forwarding,no-agent-forwarding,no-user-rc $SSH_PUBLIC_KEY
EOF

install_sudoers_candidate 0440 "/etc/sudoers.d/skeleton-runner-gateway" <<EOF
