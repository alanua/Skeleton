...
def _valid_source_issue_number(source_issue_number: int | None) -> int | None:
    if isinstance(source_issue_number, int) and source_issue_number > 0:
        return source_issue_number
    return None
    ...
    return {
        **card_payload,
        "text": _build_pr_ready_operator_text(
            pr_number,
            target_repository,
            source_issue_number=_valid_source_issue_number(source_issue_number),
    ...
    try:
        ...
        try:
            card_payload = build_done_pr_ready_card_payload(
                ...
                source_issue_number=issue_number if issue is not None else None,
            )
        except Exception:
            send_telegram_notification(plain_message)
            return
