...
_ISSUE_WORKTREE_FETCH_TIMEOUT_SECONDS = 30
...
def _issue_worktree_source_spec(
    ...
    if base_value is not _PRESENT:
        ref, reason = _issue_worktree_canonical_ref(
            base_value, bare_branch=base_alias in {"base_branch", "Base Branch"}
        )
        if reason is not None or ref is None:
            return None, reason or "invalid_source_ref"
    ...
    if base_value is _PRESENT:
        ...
        base_alias = "Base Branch" if base_value is not _PRESENT else None
    if base_value is not _PRESENT:
        ref, reason = _issue_worktree_canonical_ref(
...
def _resolve_issue_worktree_source(
    ...
    if not ok:
        return (
            None,
            "source_fetch_failed" if reason != "git_command_timeout" else reason,
        )
    try:
