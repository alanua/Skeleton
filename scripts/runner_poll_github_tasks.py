...
_PROTECTED_BRANCH_COMPONENT_RE = re.compile(r"[A-Za-z0-9._-]+")
...
def _normalize_protected_source_ref(value: object) -> tuple[str | None, str | None]:
    ...
    if (
        not branch
        or len(branch) > _PROTECTED_SOURCE_MAX_LENGTH
        or branch.endswith("/")
        or branch.endswith(".")
    ):
        return None, "reason=invalid_source_ref"
...
def _raw_direct_metadata_field(metadata: str, field: str) -> str | None:
    match = re.search(
...
def prepare_git_issue_worktree(
    ...
    if code != 0:
        return code, "reason=origin_remote_config_failed", path
    code, _output = run_command(["git", "fetch", "origin"], cwd=path)
    if code != 0:
        return code, "reason=source_fetch_failed", path
