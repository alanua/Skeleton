...
def sync_telegram_callback_poller_runtime(workdir: str) -> str:
    ...
    steps = (
        (
            "stop_callback_timer",
            ["sudo", "systemctl", "stop", TELEGRAM_CALLBACK_POLLER_TIMER],
            None,
        ),
        (
            "stop_callback_service",
            ["sudo", "systemctl", "stop", TELEGRAM_CALLBACK_POLLER_SERVICE],
            None,
        ),
    ...
    copy_and_start_steps = (
        (
            ...
            [
                ...
                "cp",
                str(source_service),
                f"/etc/systemd/system/{TELEGRAM_CALLBACK_POLLER_SERVICE}",
            ],
        ),
        (
            ...
            [
                ...
                "cp",
                str(source_timer),
                f"/etc/systemd/system/{TELEGRAM_CALLBACK_POLLER_TIMER}",
            ],
        ),
        ("systemd_daemon_reload", ["sudo", "systemctl", "daemon-reload"]),
        (
            "enable_callback_timer",
            ["sudo", "systemctl", "enable", TELEGRAM_CALLBACK_POLLER_TIMER],
        ),
        (
            "start_callback_timer",
            ["sudo", "systemctl", "start", TELEGRAM_CALLBACK_POLLER_TIMER],
        ),
        (
            "run_callback_service_once",
            ["sudo", "systemctl", "start", TELEGRAM_CALLBACK_POLLER_SERVICE],
        ),
    )
