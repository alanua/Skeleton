...
def worktree_root() -> Path:
    ...
    return Path(configured_root).expanduser() if configured_root else DEFAULT_WORKTREE_ROOT
...
def prepare_issue_worktree(
    ...
    if path.exists():
        for command in (
            ...
            output_parts.append(_format_command_output(command, output))
            if code != 0:
                return (
                    code,
            ...
            if command == ["git", "status", "--short"] and output.strip():
                return (
                    1,
            ...
            if command == ["git", "branch", "--show-current"] and output.strip() != branch:
                return (
                    1,
                    "Existing issue worktree is on the wrong branch; cleanup is required "
                    f"before reuse. Expected {branch!r}.\n\n"
                    + "\n".join(output_parts),
                    path,
                )
        return 0, "\n".join(output_parts), path
...
def prepare_issue_branch(
    ...
    code, output, path = prepare_issue_worktree(issue_number, coordinator_workdir)
    return code, output, str(path)
