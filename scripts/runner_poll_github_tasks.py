...
def _backup_graphify_profiles() -> tuple[GraphifyProfileBackup | None, str, int]:
    ...
    try:
        backup_root = Path(
            tempfile.mkdtemp(prefix="skeleton-graphify-profile-backup-")
        )
    ...
    return GraphifyProfileBackup(backup_root, tuple(entries)), "created", len(entries)
...
def _restore_graphify_profiles(backup: GraphifyProfileBackup) -> str:
    ...
    for path in _graphify_managed_profile_paths():
        if path.exists() or path.is_symlink():
            if not _remove_graphify_profile_path(path):
                return "failed"
        if path in backed_up_paths:
            backup_path = next(
                saved_path
                for original_path, saved_path in backup.entries
                if original_path == path
            )
            if not _copy_graphify_profile_path(backup_path, path):
                return "failed"
    return "restored"
...
def _run_graphify_ast_smoke(status_lines: list[str]) -> str | None:
    with tempfile.TemporaryDirectory(prefix="skeleton-graphify-smoke-") as tmp:
        ...
        status_lines.append("step=run_synthetic_ast_smoke status=done")
    return None
