...
def sync_telegram_callback_poller_runtime(workdir: str) -> str:
    ...
    steps = (
        (
            "stop_callback_timer",
            ["sudo", "-n", "systemctl", "stop", TELEGRAM_CALLBACK_POLLER_TIMER],
            None,
        ),
        (
            "stop_callback_service",
            ["sudo", "-n", "systemctl", "stop", TELEGRAM_CALLBACK_POLLER_SERVICE],
            None,
        ),
    ...
    service_unit = f"/etc/systemd/system/{TELEGRAM_CALLBACK_POLLER_SERVICE}"
    timer_unit = f"/etc/systemd/system/{TELEGRAM_CALLBACK_POLLER_TIMER}"
    copy_and_start_steps = (
        (
            "copy_callback_service_unit",
            ["sudo", "-n", "cp", str(source_service), service_unit],
        ),
        (
            "copy_callback_timer_unit",
            ["sudo", "-n", "cp", str(source_timer), timer_unit],
        ),
        (
            "own_callback_units",
            ["sudo", "-n", "chown", "root:root", service_unit, timer_unit],
        ),
        (
            "mode_callback_units",
            ["sudo", "-n", "chmod", "0644", service_unit, timer_unit],
        ),
        ("systemd_daemon_reload", ["sudo", "-n", "systemctl", "daemon-reload"]),
        (
            "enable_callback_timer",
            ["sudo", "-n", "systemctl", "enable", TELEGRAM_CALLBACK_POLLER_TIMER],
        ),
        (
            "start_callback_timer",
            ["sudo", "-n", "systemctl", "start", TELEGRAM_CALLBACK_POLLER_TIMER],
        ),
        (
            "run_callback_service_once",
            ["sudo", "-n", "systemctl", "start", TELEGRAM_CALLBACK_POLLER_SERVICE],
        ),
    )
    ...
    report = _run_maintenance_command(
        ...
        [
            ...
            "is-active",
            "--quiet",
            TELEGRAM_CALLBACK_POLLER_TIMER,
        ],
        status_lines,
    )
    if report is not None:
