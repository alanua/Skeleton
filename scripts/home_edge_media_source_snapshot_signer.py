...
from core.home_edge.media_source_snapshot import (
    SIGNER_ENVELOPE_SCHEMA,
    MAX_SIGNER_STDIN_BYTES,
    _validated_unsigned_snapshot_mapping,
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args:
        return _reject()
    try:
        data = sys.stdin.buffer.read(MAX_SIGNER_STDIN_BYTES + 1)
        if len(data) > MAX_SIGNER_STDIN_BYTES:
            return _reject()
        decoded = json.loads(data.decode("utf-8"))
        unsigned = _validated_unsigned_snapshot_mapping(decoded)
        secret = _read_exec_hmac_secret_config()
        signed = dict(unsigned)
        signed["signature"] = sign_request(HomeEdgeExecRequest.from_mapping(unsigned), secret)
        envelope = {
            "schema": SIGNER_ENVELOPE_SCHEMA,
...
def _reject() -> int:
    sys.stderr.write("snapshot signer rejected request\n")
    return 2
