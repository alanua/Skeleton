...
from core.home_edge.media_source_snapshot import build_snapshot_request_from_controller_config
...
def _caller_is_canonical_runner() -> bool:
    try:
        return pwd.getpwuid(os.geteuid()).pw_name == CANONICAL_RUNNER_SERVICE_USER
    except KeyError:
        return False
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not _caller_is_canonical_runner():
        ...
        return 2
    try:
        request = build_snapshot_request_from_controller_config()
