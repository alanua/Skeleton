...
WT="$(mktemp -d /tmp/skeleton-2808-patch.XXXXXX)"
BODY="$(mktemp)"
FOCUSED="$(mktemp)"
FULL="$(mktemp)"
trap 'git -C "$CHECKOUT" worktree remove --force "$WT" >/dev/null 2>&1 || true; rm -f "$BODY" "$FOCUSED" "$FULL"' EXIT

publish_blocked() {
  local reason="$1"
  {
    echo '### #2808 deterministic patch receipt'
    echo
    echo '```text'
    echo 'STATUS=BLOCKED'
    echo "REASON=$reason"
...
if git -C "$CHECKOUT" show-ref --verify --quiet "refs/heads/$BRANCH"; then
  publish_blocked local_branch_exists
fi
...
FOCUSED_RC=$?
set -e
test "$FOCUSED_RC" -eq 0 || publish_blocked focused_tests_failed
...
FULL_RC=$?
set -e
test "$FULL_RC" -eq 0 || publish_blocked full_tests_failed
...
git -C "$WT" commit -q -m "Fix nested RUN_NOW task payload parsing"
HEAD="$(git -C "$WT" rev-parse HEAD)"
git -C "$WT" push -q origin "HEAD:refs/heads/$BRANCH"

PR_URL="$(gh pr create --repo "$REPO" --base main --head "$BRANCH" --draft \
  --title "Fix nested RUN_NOW task payload parsing" \
  --body "Fixes the #2808 root cause on exact base $BASE. The queue replenisher now reads typed fields from payload in skeleton.runner_task.v1 while preserving top-level typed-field precedence and metadata fallback. Adds a #2785-shaped regression covering nested allowed_files, nested depends_on, satisfied runner:done dependency and RUN_NOW selection. Protected file changed; merge requires exact-head operator review.\n\nValidation: focused Runner tests PASS; full pytest PASS; py_compile PASS; git diff --check PASS.\n\nRefs #2808 #2785 #2786")"
