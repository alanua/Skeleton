...
BWS_SERVER_BASE="https://vault.bitwarden.eu"
...
# Verify exact host/user and configure the non-secret Bitwarden EU endpoint before
# asking for any secret material. Runtime BwsCliSecretsManagerStore uses HOME=/home/agent,
# so this persisted bws profile is also the canonical runtime region configuration.
if ! ssh "${SSH_OPTS[@]}" "$HOST" \
  "set -euo pipefail; test \"\$(hostname)\" = hetzner-agent-runner-1; test \"\$(id -un)\" = agent; test -x /usr/local/bin/bws; HOME=/home/agent /usr/local/bin/bws config server-base '$BWS_SERVER_BASE' >/dev/null" \
  </dev/null; then
  echo 'RESULT=BLOCKED:hetzner_or_bws_eu_preflight'
  exit 0
fi
...
printf '%s' "$PRIVATE_JSON" | ssh \
  "${SSH_OPTS[@]}" \
  "$HOST" \
  "set -euo pipefail; test \"\$(hostname)\" = hetzner-agent-runner-1; test \"\$(id -un)\" = agent; HOME=/home/agent /usr/local/bin/bws config server-base '$BWS_SERVER_BASE' >/dev/null; t=\$(mktemp /tmp/pr2814-runtime-activate.XXXXXX.py); trap 'rm -f \"\$t\"' EXIT; curl -fsSL '$HELPER_URL' -o \"\$t\"; chmod 700 \"\$t\"; exec python3 \"\$t\""
rc=$?
set -e
PRIVATE_JSON=""
unset PRIVATE_JSON
