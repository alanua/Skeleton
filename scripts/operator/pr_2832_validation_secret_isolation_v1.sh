...
HELPER_COMMIT="824ec92265235df85d2b086dc46ace38d1488b18"
HELPER_URL="https://raw.githubusercontent.com/alanua/Skeleton/${HELPER_COMMIT}/scripts/operator/pr_2832_validation_secret_isolation_remote.py"
...
set +e
ssh "${SSH_OPTS[@]}" "$HOST" \
  "set -euo pipefail; t=\$(mktemp /tmp/pr2832-validation-isolation.XXXXXX.py); trap 'rm -f \"\$t\"' EXIT; curl -fsSL '$HELPER_URL' -o \"\$t\"; chmod 700 \"\$t\"; python3 \"\$t\"" </dev/null
rc=$?
set -e
