...
HELPER_COMMIT="a6599c9c409787b4ab63780a79edec6123b2017d"
...
set +e
ssh "${SSH_OPTS[@]}" "$HOST" \
  "set -euo pipefail; t=\$(mktemp /tmp/pr2808-deterministic-v3.XXXXXX.py); trap 'rm -f \"\$t\"' EXIT; curl -fsSL '$HELPER_URL' -o \"\$t\"; chmod 700 \"\$t\"; python3 \"\$t\"" </dev/null
rc=$?
set -e
