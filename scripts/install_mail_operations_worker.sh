...
install -d -m 0700 -o "$SERVICE_USER" -g "$SERVICE_GROUP" "$STATE_ROOT/gmail"
install -m 0755 scripts/mail_operations_worker.py "$INSTALL_ROOT/mail_operations_worker.py"
install -m 0755 scripts/gmail_oauth_authorize.py "$INSTALL_ROOT/gmail_oauth_authorize.py"
cp -R adapters core integrations "$INSTALL_ROOT/"
